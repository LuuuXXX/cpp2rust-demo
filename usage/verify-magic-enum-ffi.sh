...
#
# 用途：验证 cpp2rust-demo 对 magic_enum 库的完整 Rust safe FFI 生成能力
#       （header-only、重度 constexpr / 模板元编程）
# =============================================================================
set -euo pipefail
...
DRIVER_CPP="${RUN_DIR}/enum_driver.cpp"
DRIVER_OBJ="${OBJ_DIR}/enum_driver.o"
BUILD_SCRIPT="${RUN_DIR}/build-magic-enum.sh"
...
if [ ! -f "${MAGIC_ENUM_INCLUDE}/magic_enum/magic_enum.hpp" ]; then
    fail "未找到 magic_enum.hpp：${MAGIC_ENUM_INCLUDE}/magic_enum/magic_enum.hpp
请先运行：git submodule update --init references/magic_enum"
fi
...
COMPILE_LOG="${RUN_DIR}/enum-driver.compile.log"
if g++ -c -std="${CXX_STD}" -I"${MAGIC_ENUM_INCLUDE}" "${DRIVER_CPP}" -o "${DRIVER_OBJ}" >"${COMPILE_LOG}" 2>&1; then
...
#!/usr/bin/env bash
set -e
g++ -c -std="${CXX_STD}" -I"${MAGIC_ENUM_INCLUDE}" "${DRIVER_CPP}" -o /dev/null 2>&1
EOF
...
if [ -f "${DRIVER_OBJ}" ]; then
    ...
    if [ "${DRIVER_SYMBOLS:-0}" -gt 0 ]; then
        ...
        info "驱动文件仅用于触发 header-only constexpr/模板解析；无导出符号属于预期"
    fi
else
...
if [ -d "${RUST_SRC}" ]; then
    ...
    INCLUDE_HITS=$(grep -r '#include <magic_enum/magic_enum.hpp>' "${RUST_SRC}" 2>/dev/null | wc -l | tr -d '[:space:]')
    ...
    info "magic_enum 头文件 include 命中数：${INCLUDE_HITS:-0}"
    if [ "${CPP_BLOCK_FILES:-0}" -gt 0 ] && [ "${INCLUDE_HITS:-0}" -gt 0 ]; then
        ok "生成代码已包含 magic_enum 头文件解析痕迹（header-only constexpr 解析通过）"
    else
        warn "生成代码中未找到预期的 magic_enum include / hicc::cpp! 块"
...
echo -e "
${BOLD}6c. 交叉比对${NC}"
info "magic_enum 为 header-only 驱动验证场景，无 extern-C shim，本项不适用"
...
echo ""
echo -e "${BOLD}┌─────────────────────────────────────────────────────────┐${NC}"
echo -e "${BOLD}│      cpp2rust-demo magic_enum header-only 验证结果      │${NC}"
echo -e "${BOLD}└─────────────────────────────────────────────────────────┘${NC}"
echo ""
echo -e "  ${BOLD}项目：${NC}      magic_enum（header-only，constexpr / 模板元编程）"
echo -e "  ${BOLD}feature：${NC}   ${FEATURE}"
echo -e "  ${BOLD}include：${NC}   ${MAGIC_ENUM_INCLUDE}"
...
if [ "${INCLUDE_HITS:-0}" -gt 0 ]; then
    echo -e "  ${GREEN}✓ 已验证 header-only constexpr 头文件被成功捕获与解析${NC}"
else
    echo -e "  ${RED}✗ 未在生成代码中看到 magic_enum 头文件解析结果${NC}"
    SCRIPT_ERRORS=$((SCRIPT_ERRORS + 1))
fi
