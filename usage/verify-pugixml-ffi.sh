...
#
# 用途：本地验证 cpp2rust-demo 对 pugixml C++ 库的 OOP 类 FFI 生成能力
#
# 背景说明：
#   pugixml 是单头 + 单源 XML 解析库（src/pugixml.cpp + src/pugixml.hpp），具有清晰的 xml_document/xml_node/xml_attribute OOP API，并包含迭代器类。无需 shim 层，工具直接提取 C++ 类绑定（import_class!）
#
# 流程总览：
#   § 0. 环境检查
#   § 1. 安装 cpp2rust-demo
...
#   ① OOP 类直接提取 —— 工具从 C++ 源文件提取 class 绑定
#   ② import_class! 生成 —— xml_document/xml_node/xml_attribute 等类生成 FFI 绑定块
#   ③ 迭代器类识别 —— xml_node_iterator 等迭代器类被识别
...
SOURCE_DIR="${REPO_DIR}/references/pugixml"
PUGIXML_SRC="${SOURCE_DIR}/src/pugixml.cpp"
PUGIXML_INCLUDE="${SOURCE_DIR}/src"

if [ ! -d "${SOURCE_DIR}" ] || [ ! -f "${PUGIXML_SRC}" ]; then
    info "references/pugixml 子模块未初始化，正在初始化..."
    git -C "${REPO_DIR}" submodule update --init references/pugixml || \
        fail "子模块初始化失败：references/pugixml；请检查网络或手动执行 git submodule update --init references/pugixml"
fi

if [ ! -d "${SOURCE_DIR}" ] || [ ! -f "${PUGIXML_SRC}" ]; then
    fail "未找到 pugixml 源文件：${PUGIXML_SRC}"
fi

info "pugixml 源目录：${SOURCE_DIR}"
info "pugixml 源文件：${PUGIXML_SRC}"
info "pugixml include：${PUGIXML_INCLUDE}"
ok "pugixml 源文件就绪"
...
obj="${OBJ_DIR}/pugixml.o"
if g++ -c -std="${CXX_STD}" -I"${PUGIXML_INCLUDE}" "${PUGIXML_SRC}" -o "${obj}" 2>&1; then
    ok "编译成功：pugixml.o"
else
...
set -e
g++ -c -std="${CXX_STD}" -I"${PUGIXML_INCLUDE}" "${PUGIXML_SRC}" -o /dev/null 2>&1
EOF
...
"
        ...
        PUGIXML_INCLUDE_BP="$(to_build_path "${PUGIXML_INCLUDE}")"
...
fn main() {
    ...
    cc_build.include("${PUGIXML_INCLUDE_BP}");
    cc_build.file("${PUGIXML_SRC_BP}");
    build
...
if [ -d "${RUST_SRC}" ]; then
    ...
    echo ""
    echo "──── 关键类名检查 ────"
    if grep -q "xml_document" "${RUST_SRC}" 2>/dev/null; then
        echo -e "  ${GREEN}✓ 生成代码提取到类：xml_document${NC}"
    else
        echo -e "  ${YELLOW}? 生成代码中未直接找到类：xml_document${NC}"
    fi
    if grep -q "xml_node" "${RUST_SRC}" 2>/dev/null; then
        echo -e "  ${GREEN}✓ 生成代码提取到类：xml_node${NC}"
    else
        echo -e "  ${YELLOW}? 生成代码中未直接找到类：xml_node${NC}"
    fi
    if grep -q "xml_attribute" "${RUST_SRC}" 2>/dev/null; then
        echo -e "  ${GREEN}✓ 生成代码提取到类：xml_attribute${NC}"
    else
        echo -e "  ${YELLOW}? 生成代码中未直接找到类：xml_attribute${NC}"
    fi
    if grep -q "xml_node_iterator" "${RUST_SRC}" 2>/dev/null; then
        echo -e "  ${GREEN}✓ 生成代码提取到类：xml_node_iterator${NC}"
    else
        echo -e "  ${YELLOW}? 生成代码中未直接找到类：xml_node_iterator${NC}"
    fi
else
...
if [ -s "${NM_CACHE}" ]; then
    if grep -q "xml_document" "${NM_CACHE}" 2>/dev/null; then
        echo -e "  ${GREEN}✓ nm 符号中包含：xml_document${NC}"
    else
        echo -e "  ${YELLOW}? nm 符号中未直接看到：xml_document${NC}"
    fi
    if grep -q "xml_node" "${NM_CACHE}" 2>/dev/null; then
        echo -e "  ${GREEN}✓ nm 符号中包含：xml_node${NC}"
    else
        echo -e "  ${YELLOW}? nm 符号中未直接看到：xml_node${NC}"
    fi
    if grep -q "xml_attribute" "${NM_CACHE}" 2>/dev/null; then
        echo -e "  ${GREEN}✓ nm 符号中包含：xml_attribute${NC}"
    else
        echo -e "  ${YELLOW}? nm 符号中未直接看到：xml_attribute${NC}"
    fi
    if grep -q "xml_node_iterator" "${NM_CACHE}" 2>/dev/null; then
        echo -e "  ${GREEN}✓ nm 符号中包含：xml_node_iterator${NC}"
    else
        echo -e "  ${YELLOW}? nm 符号中未直接看到：xml_node_iterator${NC}"
    fi
else
...
echo -e "${BOLD}└─────────────────────────────────────────────────────────┘${NC}"
echo ""
echo -e "  ${BOLD}项目：${NC}      pugixml（直接 OOP 类提取，无 shim 层）"
echo -e "  ${BOLD}feature：${NC}   ${FEATURE}"
echo -e "  ${BOLD}源文件：${NC}    ${PUGIXML_SRC}"
...
SKILL 适用场景（直接 OOP 类提取工作流）：
  ...
  在 pugixml 源文件目录 与 GitHub Copilot 对话，说：
    "帮我用 cpp2rust-demo 把 pugixml 源文件转换为 Rust FFI"
  或：
    "对 pugixml 项目执行 cpp2rust-demo 转换"

────────────────────────────────────────────────────────────────
OOP 类库的典型工作流（以 pugixml 为例）：

  1. 确认待分析的 C++ 源文件与头文件（如 src/pugixml.cpp / src/pugixml.hpp）

  2. 在源码目录准备可复现的 build script

  3. 运行 cpp2rust-demo init
     cpp2rust-demo init --feature pugixml_ffi -- bash build.sh

  4. 运行 cpp2rust-demo merge
     cpp2rust-demo merge --feature pugixml_ffi

  5. 在生成的 Rust 项目中使用 import_class! 绑定调用 pugixml OOP API

────────────────────────────────────────────────────────────────
⚠  重要提示：
  cpp2rust-demo 对直接 OOP 提取依赖可编译的真实 C++ 源文件。
  若只捕获到模板/声明片段，可能只生成部分 hicc::cpp! 头文件块，
  不会生成完整的 import_class! OOP 绑定。
