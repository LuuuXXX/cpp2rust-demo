...
#
# 用途：本地验证 cpp2rust-demo 对 marzer/toml++ header-only 库的 Rust safe FFI
#       生成能力（大型单头文件 + 重度模板，C++17 TOML 解析库）
#
# 背景说明：
#   toml++ 是 header-only 的 TOML 解析库（单个 include/toml++/toml.hpp，C++17，
#   大型单头文件 + 重度模板），是验证工具对「大型单头实库」解析鲁棒性的项目。
#   由于无独立 .cpp 源文件，需创建驱动文件触发头文件解析。
#
#   驱动文件定义 TomlWrapper 类，方法签名只使用标量或 std 类型
#   （不引用 toml 内部类型），确保生成产物可编译。
#
# 流程总览：
#   § 0. 环境检查
#   § 1. 安装 cpp2rust-demo
#   § 2. 定位 toml++ 源码（references/tomlplusplus 子模块 或 临时克隆）
...
#
# 可配置环境变量：
#   FEATURE       （默认 tomlplusplus）  cpp2rust-demo feature 名称
#   SKIP_INSTALL  （默认 0）             置 1 跳过 cargo install
#   CXX_STD       （默认 c++17）         C++ 标准版本（toml++ 要求 C++17）
#
# 用法示例：
#   bash usage/verify-tomlplusplus-ffi.sh
#   SKIP_INSTALL=1 bash usage/verify-tomlplusplus-ffi.sh
#   FEATURE=my_toml bash usage/verify-tomlplusplus-ffi.sh
#
# 系统要求（Ubuntu/Debian）：
...
SUBMODULE_DIR="${REPO_DIR}/references/tomlplusplus"
CLONE_DIR="${TMPDIR}/tomlplusplus-ffi-demo"

if [ -f "${SUBMODULE_DIR}/include/toml++/toml.hpp" ]; then
    TOMLPLUSPLUS_DIR="${SUBMODULE_DIR}"
    info "使用仓库子模块：${TOMLPLUSPLUS_DIR}"
else
    info "子模块未初始化，克隆 toml++ 到 ${CLONE_DIR}…"
    if [ -d "${CLONE_DIR}" ]; then
        info "目录已存在，尝试 git pull…"
        git -C "${CLONE_DIR}" pull --ff-only 2>/dev/null || true
    else
        git clone --depth 1 https://github.com/marzer/tomlplusplus.git "${CLONE_DIR}"
    fi
    TOMLPLUSPLUS_DIR="${CLONE_DIR}"
fi

if [ ! -f "${TOMLPLUSPLUS_DIR}/include/toml++/toml.hpp" ]; then
    fail "未找到 include/toml++/toml.hpp：${TOMLPLUSPLUS_DIR}/include/toml++/toml.hpp"
fi

TOMLPLUSPLUS_INCLUDE="${TOMLPLUSPLUS_DIR}/include"
info "toml++ 源码目录：${TOMLPLUSPLUS_DIR}"
...
DRIVER_CPP="${DRIVER_DIR}/toml_driver.cpp"
cat > "${DRIVER_CPP}" << 'DRIVER_EOF'
// toml++ 驱动文件 — 用于测试大型单头实库的解析能力
// 方法仅声明，签名用标量/std 类型（不引用 toml 类型），保证生成绑定可编译
...
OBJ_FILE="${OBJ_DIR}/toml_driver.o"
COMPILE_LOG="${OBJ_DIR}/compile.log"

if g++ -c -std="${CXX_STD}" \
       -DTOML_HEADER_ONLY=1 \
       -I"${TOMLPLUSPLUS_INCLUDE}" \
       "${DRIVER_CPP}" \
       -o "${OBJ_FILE}" >"${COMPILE_LOG}" 2>&1; then
    ok "toml_driver.cpp 编译成功 → ${OBJ_FILE}"
else
    warn "toml_driver.cpp 编译失败（大型单头展开可能超时），nm 验证可能不完整"
    cat "${COMPILE_LOG}" >&2
fi
...
info "构建命令：g++ -c -std=${CXX_STD} -DTOML_HEADER_ONLY=1 -I${TOMLPLUSPLUS_INCLUDE} ${DRIVER_CPP}"
...
fn main() {
    ...
    // toml++ 头文件包含路径（include/ 目录）
    cc_build.include("${TOMLPLUSPLUS_INCLUDE_BP}");
    // 启用 header-only 模式（TOML_HEADER_ONLY=1）
    cc_build.define("TOML_HEADER_ONLY", "1");
    // 编译驱动文件（触发 toml.hpp 模板展开，提供类绑定入口）
...
EOF
        ...
        sed 's/^/    /' "${BUILD_RS}"
        ok "build.rs 已就地补全（注入头路径 + TOML_HEADER_ONLY + 驱动文件 + 逐文件注册）"
    fi
else
...
if [ -f "${SMOKE_FILE}" ]; then
    ...
    info "未生成 tests/smoke.rs（header-only 驱动类可能无可往返测试的标量属性），跳过 cargo test"
fi
...
# ── 6a. nm 查看 .o 中的 T 段符号 ─────────────────────────────────────────────
echo -e "\n${BOLD}6a. toml_driver 目标文件符号（nm 验证）${NC}"
NM_CACHE=$(mktemp)
if [ -f "${OBJ_FILE}" ]; then
...
if [ "${T_COUNT}" -gt 0 ]; then
    ...
    warn "未找到 T 段符号（驱动文件是否编译成功？大型单头展开可能导致编译超时）"
fi
...
if [ -d "${RUST_SRC}" ]; then
    ...
    if [ "${IMPORT_CLASS_FILES}" -gt 0 ]; then
        ...
        # 检查驱动类 TomlWrapper 是否被提取
        if grep -rl "TomlWrapper" "${RUST_SRC}" 2>/dev/null | grep -q .; then
            echo -e "  ${GREEN}✓ 找到 TomlWrapper 类绑定${NC}"
        else
            info "  未找到 TomlWrapper（可能被识别为 template_base 类型）"
        fi
    else
        info "未找到 import_class! 块（header-only 大型模板库属正常现象，工具已成功解析头文件）"
    fi
else
...
echo -e "${BOLD}└─────────────────────────────────────────────────────────┘${NC}"
echo ""
echo -e "  ${BOLD}项目：${NC}      toml++（header-only，大型单头 C++17 TOML 解析库）"
echo -e "  ${BOLD}feature：${NC}   ${FEATURE}"
echo -e "  ${BOLD}源码目录：${NC}  ${TOMLPLUSPLUS_DIR}"
...
if [ -d "${RUST_SRC}" ]; then
    ...
    if [ "${IMPORT_CLASS_FILES}" -gt 0 ]; then
        ...
        echo -e "  ${YELLOW}⚠ 未找到 import_class! 块（header-only 大型模板库属正常现象，工具已正确解析头文件）${NC}"
    fi
fi
