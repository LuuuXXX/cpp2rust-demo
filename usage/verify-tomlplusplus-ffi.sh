#!/usr/bin/env bash
# =============================================================================
# verify-tomlplusplus-ffi.sh — 用途：验证 cpp2rust-demo 对 toml++ (tomlplusplus) 的 FFI 生成能力
# 项目特征：header-only（include/toml++/toml.h ~30K+ 行），重度 C++17 模板
# 用 driver cpp 触发预处理拦截。流程与 verify-nlohmann-json-ffi.sh 同构。
# =============================================================================
...
# toml++ 的若干方法返回引用/迭代器或有 ref-qualifier 重载（& 和 &&），hicc 无法
# 正确绑定（误识别返回类型或无法消歧重载）。在 cargo check 前批量过滤这些方法。
cpp2rust_filter_bindings "${RUST_SRC}" \
    "begin" "end" "cbegin" "cend" "rbegin" "rend" \
    "flatten" "prune" "data" "front" "back" "at" \
    "insert" "erase" "emplace" "extract" "merge"
