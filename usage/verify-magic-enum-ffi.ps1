#!/usr/bin/env pwsh
# =============================================================================
# verify-magic-enum-ffi.ps1  (Windows PowerShell)
#
# 用途：本地验证 cpp2rust-demo 对 magic_enum C++ 库的完整 Rust safe FFI 生成能力
#
# 流程总览：
#   § 0. 环境检查
#   § 1. 安装 cpp2rust-demo
...
#   § 7. 生成结果汇报
#
# 说明：magic_enum 是 header-only 元编程库，通过 ColorWrapper 驱动类触发类型提取。
# 重点检查 build.rs include 注入，以及 import_class! 是否按预期生成。
#
# 系统要求（Windows）：
...
# ─── 可配置参数 ───────────────────────────────────────────────────────────────
$Feature     = if ($env:FEATURE) { $env:FEATURE } else { "magic_enum_ffi" }
...
try {
    ...
    Step "§ 2. 检查 magic_enum 子模块"
    $MagicEnumDir = Join-Path $RepoDir 'references/magic_enum'
    $MagicEnumInclude = Join-Path $MagicEnumDir 'include'
    if (-not (Test-Path (Join-Path $MagicEnumInclude 'magic_enum/magic_enum.hpp'))) {
        Fail 'magic_enum 子模块未初始化，请运行：git submodule update --init references/magic_enum'
    }
    Ok 'magic_enum 子模块检查通过'

    Step "§ 3. 创建并编译驱动 .cpp（供 nm / 预处理验证）"
    $ObjDir = Join-Path $RuntimeDir 'obj'
    $null = New-Item -ItemType Directory -Force -Path $ObjDir
    $DriverCpp = Join-Path $RuntimeDir 'magic_enum_driver.cpp'
    @'
...
'@ | Set-Content -Path $DriverCpp -Encoding utf8
    $ObjFile = Join-Path $ObjDir 'magic_enum_driver.obj'
    $CompileLog = Join-Path $ObjDir 'magic_enum_driver.log'
    $compileExit = Invoke-CxxCompile -Compiler $Compiler -Std $CxxStd -IncludeDirs @($MagicEnumInclude) -Sources @($DriverCpp) -Output $ObjFile -LogFile $CompileLog
    if ($compileExit -ne 0) { Warn '驱动文件编译失败'; $script:ScriptErrors++ } else { Ok '驱动文件编译成功' }
    $ObjectFiles = @($ObjFile)

    Step "§ 4. cpp2rust-demo init（捕获 FFI 脚手架）"
    $BuildScript = Join-Path $RuntimeDir 'build-magic-enum.ps1'
    Write-CompileBuildScript -Path $BuildScript -Compiler $Compiler -Std $CxxStd -IncludeDirs @($MagicEnumInclude) -SourceFiles @($DriverCpp) -OutputDir (Join-Path $RuntimeDir 'build')
    ...
    if (Test-Path $BuildRs) {
        ...
        if ($BuildRsText -notmatch 'cc_build\.include') {
            Write-FallbackBuildRs -BuildRs $BuildRs -RustProject $RustProject -LibName $LibName -Std $CxxStd -IncludeDirs @($MagicEnumInclude) -CommentLines @('header-only 场景仅需 include 路径','驱动类 ColorWrapper 用于验证 import_class! 提取') -LinkStdCpp
        }
    }
    ...
    Info 'magic_enum 为 header-only hicc-direct 场景，不做 shim 符号交叉比对。'
