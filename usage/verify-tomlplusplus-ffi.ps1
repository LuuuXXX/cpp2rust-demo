#!/usr/bin/env pwsh
# =============================================================================
# verify-tomlplusplus-ffi.ps1  (Windows PowerShell)
#
# 用途：本地验证 cpp2rust-demo 对 tomlplusplus C++ 库的完整 Rust safe FFI 生成能力
#
# 流程总览：
#   § 0. 环境检查
#   § 1. 安装 cpp2rust-demo
#   § 2. 检查 tomlplusplus 子模块
...
#   § 7. 生成结果汇报
#
# 说明：tomlplusplus 是 header-only 库，通过定义 TOML_HEADER_ONLY=1 的驱动文件触发类型提取。
# 重点检查 build.rs include/define 注入，以及 import_class! 是否按预期生成。
#
# 系统要求（Windows）：
...
# ─── 可配置参数 ───────────────────────────────────────────────────────────────
$Feature     = if ($env:FEATURE) { $env:FEATURE } else { "tomlplusplus_ffi" }
...
try {
    ...
    Step "§ 2. 检查 tomlplusplus 子模块"
    $TomlDir = Join-Path $RepoDir 'references/tomlplusplus'
    $TomlInclude = Join-Path $TomlDir 'include'
    if (-not (Test-Path (Join-Path $TomlInclude 'toml++/toml.hpp'))) {
        Fail 'tomlplusplus 子模块未初始化，请运行：git submodule update --init references/tomlplusplus'
    }
    Ok 'tomlplusplus 子模块检查通过'

    Step "§ 3. 创建并编译驱动 .cpp（供 nm / 预处理验证）"
    $ObjDir = Join-Path $RuntimeDir 'obj'
    $null = New-Item -ItemType Directory -Force -Path $ObjDir
    $DriverCpp = Join-Path $RuntimeDir 'tomlplusplus_driver.cpp'
    @'
...
'@ | Set-Content -Path $DriverCpp -Encoding utf8
    $TomlDefine = Get-DefineArg $Compiler 'TOML_HEADER_ONLY=1'
    $ObjFile = Join-Path $ObjDir 'tomlplusplus_driver.obj'
    $CompileLog = Join-Path $ObjDir 'tomlplusplus_driver.log'
    $compileExit = Invoke-CxxCompile -Compiler $Compiler -Std $CxxStd -IncludeDirs @($TomlInclude) -Sources @($DriverCpp) -Output $ObjFile -LogFile $CompileLog -ExtraArgs @($TomlDefine)
    if ($compileExit -ne 0) { Warn '驱动文件编译失败'; $script:ScriptErrors++ } else { Ok '驱动文件编译成功' }
    $ObjectFiles = @($ObjFile)

    Step "§ 4. cpp2rust-demo init（捕获 FFI 脚手架）"
    $BuildScript = Join-Path $RuntimeDir 'build-tomlplusplus.ps1'
    Write-CompileBuildScript -Path $BuildScript -Compiler $Compiler -Std $CxxStd -IncludeDirs @($TomlInclude) -SourceFiles @($DriverCpp) -OutputDir (Join-Path $RuntimeDir 'build') -ExtraArgs @($TomlDefine)
    ...
    if (Test-Path $BuildRs) {
        ...
        if ($BuildRsText -notmatch 'cc_build\.include') {
            Write-FallbackBuildRs -BuildRs $BuildRs -RustProject $RustProject -LibName $LibName -Std $CxxStd -IncludeDirs @($TomlInclude) -ExtraLines @('cc_build.define("TOML_HEADER_ONLY", Some("1"));') -CommentLines @('header-only 场景仅需 include 路径','fallback build.rs 同步定义 TOML_HEADER_ONLY=1') -LinkStdCpp
        }
    }
    ...
    if ($ImportClassFiles -le 0) { Warn '未检测到 import_class! 类绑定'; $script:ScriptErrors++ } else { Ok '已检测到 import_class! 类绑定' }
    Info 'tomlplusplus 为 header-only hicc-direct 场景，不做 shim 符号交叉比对。'
