...
#
# 用途：本地验证 cpp2rust-demo 对 nlohmann/json header-only 库的 FFI 生成能力
#
# 背景说明：
#   nlohmann/json 是 header-only 库（单个 json.hpp ~23K 行），重度使用模板。本脚本以「驱动文件」模式演示 header-only 库的工作流：编写包含库头文件的驱动 .cpp，工具提取驱动中定义的类绑定（import_class!）
#
# 流程总览：
#   § 0. 环境检查
#   § 1. 安装 cpp2rust-demo
...
#   ① 驱动文件提取 —— 工具从包含 json.hpp 的驱动文件提取类绑定
#   ② import_class! 生成 —— JsonWrapper 等类生成完整的 FFI 绑定块
#   ③ 模板头文件兼容 —— header-only + 重模板场景可被解析
...
SOURCE_DIR="${REPO_DIR}/references/nlohmann-json"
NLOHMANN_INCLUDE="${SOURCE_DIR}/include"

if [ ! -d "${SOURCE_DIR}" ] || [ ! -d "${NLOHMANN_INCLUDE}" ]; then
    info "references/nlohmann-json 子模块未初始化，正在初始化..."
    git -C "${REPO_DIR}" submodule update --init references/nlohmann-json || \
        fail "子模块初始化失败：references/nlohmann-json；请检查网络或手动执行 git submodule update --init references/nlohmann-json"
fi
...
if g++ -c -std="${CXX_STD}" -I"${NLOHMANN_INCLUDE}" "${DRIVER_TMP}" -o "${OBJ_DIR}/json_driver.o" 2>&1; then
    ok "nlohmann/json driver 编译成功"
else
    warn "编译失败（json.hpp 展开较慢），nm 验证可能不完整"
fi
...
set -e
g++ -c -std="${CXX_STD}" -I"${NLOHMANN_INCLUDE}" "${DRIVER_TMP}" -o /dev/null 2>&1
EOF
...
if [ "${CPP_SYMBOLS}" -gt 0 ]; then
    ...
    ok "已捕获 nlohmann/json 驱动文件 的 C++ 符号"
else
...
if [ -d "${RUST_SRC}" ]; then
    ...
    echo ""
    echo "──── 关键类名检查 ────"
    if grep -q "JsonWrapper" "${RUST_SRC}" 2>/dev/null; then
        echo -e "  ${GREEN}✓ 生成代码提取到类：JsonWrapper${NC}"
    else
        echo -e "  ${YELLOW}? 生成代码中未直接找到类：JsonWrapper${NC}"
    fi
else
...
if [ -s "${NM_CACHE}" ]; then
    if grep -q "JsonWrapper" "${NM_CACHE}" 2>/dev/null; then
        echo -e "  ${GREEN}✓ nm 符号中包含：JsonWrapper${NC}"
    else
        echo -e "  ${YELLOW}? nm 符号中未直接看到：JsonWrapper${NC}"
    fi
else
...
echo -e "${BOLD}└─────────────────────────────────────────────────────────┘${NC}"
echo ""
echo -e "  ${BOLD}项目：${NC}      nlohmann/json（header-only + 驱动文件模式）"
...
────────────────────────────────────────────────────────────────
SKILL 适用场景（header-only 驱动文件工作流）：

  在 nlohmann/json 驱动文件目录 与 GitHub Copilot 对话，说：
    "帮我用 cpp2rust-demo 把 nlohmann/json 驱动文件转换为 Rust FFI"
  或：
    "对这个 nlohmann/json driver 执行 cpp2rust-demo 转换"

────────────────────────────────────────────────────────────────
header-only 库的典型驱动文件工作流（以 nlohmann/json 为例）：

  1. 编写包含 json.hpp 的驱动 .cpp，并声明待提取的包装类

  2. 在驱动文件目录准备可复现的 build script

  3. 运行 cpp2rust-demo init
     cpp2rust-demo init --feature nlohmann_json_ffi -- bash build.sh

  4. 运行 cpp2rust-demo merge
     cpp2rust-demo merge --feature nlohmann_json_ffi

  5. 在生成的 Rust 项目中使用 import_class! 绑定调用 JsonWrapper / JSON API

────────────────────────────────────────────────────────────────
⚠  重要提示：
  cpp2rust-demo 解析 header-only 大模板库时，依赖驱动文件提供可编译入口。
  若驱动文件未声明可提取的类，可能只生成 hicc::cpp! 片段，
