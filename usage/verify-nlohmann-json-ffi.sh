...
#
# 用途：本地验证 cpp2rust-demo 对 nlohmann/json 库的完整 Rust safe FFI 生成能力
#
# 背景说明：
#   nlohmann/json 是 header-only C++ 模板库，核心头文件 json.hpp 规模很大。
#   本脚本通过一个最小驱动 .cpp 触发模板展开，验证 cpp2rust-demo 对「超大头文
#   件 + header-only + 模板类提取」工作流的支持能力：驱动编译 → init → merge →
#   cargo check/test → 检查生成的 Rust FFI 与 build.rs 注入。
#
# 流程总览：
#   § 0. 环境检查
#   § 1. 安装 cpp2rust-demo
...
#   § 5b. cargo check
#   § 5c. cargo test
#   § 6. FFI 验证（import_class!、最小符号、预处理体积）
#   § 7. 生成结果汇报
# =============================================================================
...
NLOHMANN_DIR="${REPO_DIR}/references/nlohmann-json"
NLOHMANN_INCLUDE="${NLOHMANN_DIR}/include"
if [ ! -d "${NLOHMANN_DIR}" ] || [ ! -f "${NLOHMANN_INCLUDE}/nlohmann/json.hpp" ]; then
    fail "nlohmann-json 子模块未初始化，请运行：git submodule update --init references/nlohmann-json"
fi

info "nlohmann-json 根目录：${NLOHMANN_DIR}"
info "include 目录：${NLOHMANN_INCLUDE}"
...
DRIVER_CPP="${RUNTIME_DIR}/json_driver.cpp"
mkdir -p "${OBJ_DIR}"

cat > "${DRIVER_CPP}" <<'EOFDRIVE'
...
if g++ -c -std="${CXX_STD}" -I"${NLOHMANN_INCLUDE}" "${DRIVER_CPP}" \
    -o "${OBJ_DIR}/nlohmann_driver.o" 2>/dev/null; then
    ok "驱动文件编译成功"
else
...
BUILD_SCRIPT="${RUNTIME_DIR}/build-nlohmann.sh"
cat > "${BUILD_SCRIPT}" <<EOFSCRIPT
#!/bin/bash
g++ -c -std="${CXX_STD}" \
    -I"${NLOHMANN_INCLUDE}" \
    "${DRIVER_CPP}" -o /dev/null 2>&1 || true
EOFSCRIPT
...
"
        ...
        NLOHMANN_INCLUDE_BP="$(to_build_path "${NLOHMANN_INCLUDE}")"
...
fn main() {
    ...
    cc_build.include("${NLOHMANN_INCLUDE_BP}");
    build
...
EOF
        ...
        sed 's/^/    /' "${BUILD_RS}"
        ok "build.rs 已就地补全（仅注入 nlohmann include + 逐文件注册）"
    fi
else
...
if [ -f "${SMOKE_FILE}" ]; then
    ...
    if (cd "${RUST_PROJECT}" && cargo test 2>&1); then
        ...
        warn "cargo test 失败 — 请检查上方日志（header-only 模板库可能更易触发降级）"
    fi
else
...
echo -e "\n${BOLD}6a. 驱动目标文件符号（nm 验证）${NC}"
NM_CACHE="${RUNTIME_DIR}/nlohmann-nm.txt"
find "${OBJ_DIR}" -name "*.o" 2>/dev/null \
    | xargs -r nm -C --defined-only -f posix 2>/dev/null > "${NM_CACHE}" || true

DEFINED_COUNT=$(awk 'NF > 0 {count++} END {print count+0}' "${NM_CACHE}" 2>/dev/null)
JSON_SYMBOL_COUNT=$(grep -c 'JsonWrapper\|nlohmann::\|jsonwrap_ns::' "${NM_CACHE}" 2>/dev/null || echo 0)
info "已定义符号总数：${DEFINED_COUNT}"
info "JsonWrapper / nlohmann 相关符号数：${JSON_SYMBOL_COUNT}"
if [ "${DEFINED_COUNT}" -gt 0 ]; then
    echo "──── 部分已定义符号（前 20 条）────"
    awk '{print $1}' "${NM_CACHE}" | head -20 || true
else
    warn "nm 未发现明显导出符号（header-only 驱动对象为空属于可接受现象）"
fi
...
if [ -d "${RUST_SRC}" ]; then
    ...
    CPP_BLOCK_FILES="${CPP_BLOCK_FILES:-0}"
    info "包含 import_class! 绑定的文件数：${IMPORT_CLASS_FILES}"
    info "包含 hicc::cpp! 块的文件数：${CPP_BLOCK_FILES}"

    echo "──── import_class! / hicc::cpp! 命中（前 20 条）────"
    grep -rn 'hicc::import_class!\|hicc::cpp!' "${RUST_SRC}" 2>/dev/null | head -20 || true
else
    warn "Rust 源码目录不存在：${RUST_SRC}"
fi

echo -e "\n${BOLD}6c. shim 交叉比对说明${NC}"
info "nlohmann/json 为 header-only hicc-direct 场景，本脚本不做 shim 符号交叉比对（6c 不适用）"
...
echo -e "${BOLD}└─────────────────────────────────────────────────────────┘${NC}"
echo ""
echo -e "  ${BOLD}项目：${NC}      nlohmann/json（header-only + 超大头文件）"
echo -e "  ${BOLD}feature：${NC}   ${FEATURE}"
echo -e "  ${BOLD}include：${NC}   ${NLOHMANN_INCLUDE}"
echo -e "  ${BOLD}驱动文件：${NC}   ${DRIVER_CPP}"
...
if [ -d "${RUST_SRC}" ]; then
    ...
    CPP_BLOCK_FILES=$(grep -rl "hicc::cpp!" "${RUST_SRC}" 2>/dev/null | wc -l | tr -d '[:space:]') || CPP_BLOCK_FILES=0
    IMPORT_CLASS_FILES="${IMPORT_CLASS_FILES:-0}"
    CPP_BLOCK_FILES="${CPP_BLOCK_FILES:-0}"
    echo -e "  ${BOLD}import_class! 文件数：${NC}  ${IMPORT_CLASS_FILES}"
    echo -e "  ${BOLD}hicc::cpp! 文件数：${NC}     ${CPP_BLOCK_FILES}"
    if [ "${CPP_BLOCK_FILES}" -gt 0 ]; then
        echo -e "  ${GREEN}✓ 已生成 nlohmann/json Rust FFI 脚手架${NC}"
    else
        echo -e "  ${RED}✗ 未检测到 hicc::cpp! / import_class! 输出${NC}"
...
echo -e "    find ${CPP2RUST_OUTPUT}/rust/src -name '*.rs' | xargs grep -n 'import_class\|hicc::cpp'"
echo ""
