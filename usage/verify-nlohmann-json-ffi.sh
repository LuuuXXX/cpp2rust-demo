...
#
# 用途：本地验证 cpp2rust-demo 对 nlohmann/json header-only 库的 Rust safe FFI
#       生成能力（超大单头文件，重度模板）
#
# 背景说明：
#   nlohmann/json 是 header-only 库（单个 include/nlohmann/json.hpp，~23K 行），
#   重度使用模板和 template<typename T> 特化，是验证工具对超大头文件解析能力
#   的核心项目。由于无独立 .cpp 源文件，需创建驱动文件触发头文件解析。
#
#   驱动文件定义一个简单的 JsonWrapper 类，其方法签名只使用标量或 std 类型
#   （不引用 nlohmann 内部类型），确保生成产物可编译。
#
# 流程总览：
#   § 0. 环境检查
#   § 1. 安装 cpp2rust-demo
#   § 2. 定位 nlohmann/json 源码（references/nlohmann-json 子模块 或 临时克隆）
...
#
# 可配置环境变量：
#   FEATURE       （默认 nlohmann_json）  cpp2rust-demo feature 名称
...
#   FEATURE=my_json bash usage/verify-nlohmann-json-ffi.sh
#
# 系统要求（Ubuntu/Debian）：
...
# =============================================================================
# § 2. 定位 nlohmann/json 源码
# =============================================================================
step "§ 2. 定位 nlohmann/json 源码"

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_DIR="$(git -C "${SCRIPT_DIR}" rev-parse --show-toplevel 2>/dev/null || echo "${SCRIPT_DIR}/..")"
REPO_DIR="$(cd "${REPO_DIR}" && pwd)"

SUBMODULE_DIR="${REPO_DIR}/references/nlohmann-json"
CLONE_DIR="${TMPDIR}/nlohmann-json-ffi-demo"

if [ -f "${SUBMODULE_DIR}/include/nlohmann/json.hpp" ]; then
    NLOHMANN_DIR="${SUBMODULE_DIR}"
    info "使用仓库子模块：${NLOHMANN_DIR}"
else
    info "子模块未初始化，克隆 nlohmann/json 到 ${CLONE_DIR}…"
    if [ -d "${CLONE_DIR}" ]; then
        info "目录已存在，尝试 git pull…"
        git -C "${CLONE_DIR}" pull --ff-only 2>/dev/null || true
    else
        git clone --depth 1 https://github.com/nlohmann/json.git "${CLONE_DIR}"
    fi
    NLOHMANN_DIR="${CLONE_DIR}"
fi

if [ ! -f "${NLOHMANN_DIR}/include/nlohmann/json.hpp" ]; then
    fail "未找到 include/nlohmann/json.hpp：${NLOHMANN_DIR}/include/nlohmann/json.hpp"
fi

NLOHMANN_INCLUDE="${NLOHMANN_DIR}/include"
info "nlohmann/json 源码目录：${NLOHMANN_DIR}"
info "nlohmann/json include：${NLOHMANN_INCLUDE}"
ok "nlohmann/json 源码就绪"
...
DRIVER_CPP="${DRIVER_DIR}/json_driver.cpp"
cat > "${DRIVER_CPP}" << 'DRIVER_EOF'
// nlohmann/json 驱动文件 — 用于测试模板类提取能力
// 方法仅声明，签名用标量/std 类型（不引用 nlohmann 内部类型），保证生成绑定可编译
...
OBJ_FILE="${OBJ_DIR}/json_driver.o"
...
if g++ -c -std="${CXX_STD}" \
    ...
    ok "json_driver.cpp 编译成功 → ${OBJ_FILE}"
else
    warn "json_driver.cpp 编译失败（超大头文件展开可能超时或内存不足），nm 验证可能不完整"
    cat "${COMPILE_LOG}" >&2
fi
...
info "构建命令：g++ -c -std=${CXX_STD} -I${NLOHMANN_INCLUDE} ${DRIVER_CPP}"
...
fn main() {
    ...
    // nlohmann/json 头文件包含路径（include/ 目录）
    cc_build.include("${NLOHMANN_INCLUDE_BP}");
    // 编译驱动文件（触发 json.hpp 模板展开，提供类绑定入口）
...
# ── 6a. nm 查看 .o 中的 T 段符号 ─────────────────────────────────────────────
echo -e "\n${BOLD}6a. json_driver 目标文件符号（nm 验证）${NC}"
NM_CACHE=$(mktemp)
if [ -f "${OBJ_FILE}" ]; then
...
if [ "${T_COUNT}" -gt 0 ]; then
    ...
    warn "未找到 T 段符号（驱动文件是否编译成功？超大头文件可能导致编译超时）"
fi
...
if [ -d "${RUST_SRC}" ]; then
    ...
    if [ "${IMPORT_CLASS_FILES}" -gt 0 ]; then
        ...
        info "未找到 import_class! 块（header-only 驱动类可能被识别为 template_base，属正常现象）"
    fi
else
...
# ── 6d. 预处理文件行数统计（超大头文件关键指标）────────────────────────────────
echo -e "\n${BOLD}6d. 捕获的 .cpp2rust 预处理文件行数统计（超大头文件关键指标）${NC}"
...
if [ -d "${C_DIR}" ]; then
    ...
    if [ "${TOTAL_LINES}" -gt 50000 ]; then
        ok "预处理文件行数 > 50000，证明超大头文件（json.hpp ~23K 行）展开成功 ✓"
    else
        warn "预处理文件行数较少（json.hpp 展开可能不完整）"
    fi
...
echo -e "${BOLD}└─────────────────────────────────────────────────────────┘${NC}"
echo ""
echo -e "  ${BOLD}项目：${NC}      nlohmann/json（header-only，超大单头 ~23K 行）"
echo -e "  ${BOLD}feature：${NC}   ${FEATURE}"
echo -e "  ${BOLD}源码目录：${NC}  ${NLOHMANN_DIR}"
...
if [ -d "${RUST_SRC}" ]; then
    ...
    if [ "${IMPORT_CLASS_FILES}" -gt 0 ]; then
        ...
        echo -e "  ${YELLOW}⚠ 未找到 import_class! 块（header-only 超大模板头文件属正常现象）${NC}"
    fi
fi
