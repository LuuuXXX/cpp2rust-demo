#!/usr/bin/env pwsh
# =============================================================================
# verify-pugixml-ffi.ps1  (Windows PowerShell)
#
...
#   § 7. 生成结果汇报
#
# 说明：pugixml 走 hicc-direct（无 shim）流程，直接以 src/pugixml.cpp 为输入。
# 重点检查 build.rs 是否注入源码/头路径，以及 import_class! 是否成功生成。
#
# 系统要求（Windows）：
...
# ─── 可配置参数 ───────────────────────────────────────────────────────────────
$Feature     = if ($env:FEATURE) { $env:FEATURE } else { "pugixml_ffi" }
...
try {
    ...
    $LibDir = Join-Path $RepoDir 'references/pugixml'
    $PugixmlSrc = Join-Path $LibDir 'src'
    $SourceFile = Join-Path $PugixmlSrc 'pugixml.cpp'
    if (-not (Test-Path $SourceFile)) { Fail 'pugixml 子模块未初始化，请运行：git submodule update --init references/pugixml' }
    Info "库目录：$LibDir"
    Info "源码目录：$PugixmlSrc"
    Info "源文件：$SourceFile"
    Ok 'pugixml 源码就绪'
    Step "§ 3. 编译库目标文件（供 nm 符号验证）"
    $ObjDir = Join-Path $RuntimeDir 'obj'
    $null = New-Item -ItemType Directory -Force -Path $ObjDir
    $ObjFile = Join-Path $ObjDir 'pugixml.obj'
    $CompileLog = Join-Path $ObjDir 'pugixml.log'
    $compileExit = Invoke-CxxCompile -Compiler $Compiler -Std $CxxStd -IncludeDirs @($PugixmlSrc) -Sources @($SourceFile) -Output $ObjFile -LogFile $CompileLog
    if ($compileExit -eq 0) { Ok '编译成功：pugixml.obj' } else { Warn '编译失败：src/pugixml.cpp'; if (Test-Path $CompileLog) { Get-Content $CompileLog | Write-Host } }
    $ObjectFiles = @($ObjFile)

    Step "§ 4. cpp2rust-demo init（捕获 FFI 脚手架）"
    $BuildScript = Join-Path $RuntimeDir 'build-pugixml.ps1'
    Write-CompileBuildScript -Path $BuildScript -Compiler $Compiler -Std $CxxStd -IncludeDirs @($PugixmlSrc) -SourceFiles @($SourceFile) -OutputDir (Join-Path $RuntimeDir 'build')
    ...
    if (Test-Path $BuildRs) {
        ...
        if ($BuildRsText -match 'cc_build\.include' -and $BuildRsText -match 'cc_build\.file') { Ok 'build.rs 已由工具自动注入头路径 + 库实现（方案 A 生效）' }
            ...
            Write-FallbackBuildRs -BuildRs $BuildRs -RustProject $RustProject -LibName $LibName -Std $CxxStd -IncludeDirs @($PugixmlSrc) -SourceFiles @($SourceFile) -CommentLines @('与库源码编译保持一致的 C++ 标准','① pugixml 头文件包含路径（hicc 生成的胶水 C++ #include 需要）','② 编译 src/pugixml.cpp，提供底层 C++ 实现符号') -LinkStdCpp
    ...
    Step "§ 6. FFI 验证"
    Write-Host "`n6a. 库目标文件核心符号（nm 验证）"
    $NmOutput = Get-DefinedSymbols -NmTool $NmTool -ObjectFiles $ObjectFiles
    $MatchedSymbols = @($NmOutput | Where-Object { $_ -match 'xml_document|xml_node|XMLDocument|XMLNode' })
    Info "库目标文件已定义符号数：$($NmOutput.Count)"
    Info "匹配 xml_document/xml_node 的符号数：$($MatchedSymbols.Count)"
    if ($MatchedSymbols.Count -gt 0) { $MatchedSymbols | Select-Object -First 20 | ForEach-Object { Write-Host "  $_" }; Ok '目标文件中存在 xml_document/xml_node 相关符号' } else { Warn '未在 pugixml.obj 中找到 xml_document/xml_node 相关符号'; $script:ScriptErrors++ }
    ...
    if ($ImportClassFiles -gt 0) { Ok '生成代码包含 import_class! 块（类绑定存在）✓' } else { Warn '未检测到 import_class! 块（对于 pugixml 类库这通常不符合预期）'; $script:ScriptErrors++ }
    Write-Host "`n6c. hicc-direct 说明"
    Info 'pugixml 为 hicc-direct（无 shim）场景，本脚本不做 shim 符号交叉比对。'
    ...
    Step "§ 7. 生成结果汇报"
    Write-Host ''
    Write-Host '  项目：      pugixml（hicc 直出，无 shim）'
    ...
    if ($ImportClassFiles -gt 0) { Ok '已生成 pugixml Rust safe FFI 脚手架' } else { Warn '未检测到 import_class! 绑定' }
    Ok '验证脚本执行完毕！'
