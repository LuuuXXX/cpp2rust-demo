...
"
        ...
        while IFS= read -r cpp; do
            cpp_bp="$(to_build_path "${cpp}")"
            SHIM_FILE_LINES="${SHIM_FILE_LINES}    cc_build.file(\"${cpp_bp}\");
"
        done < <(find "${SHIM_DIR}" -maxdepth 1 -name "*.cpp" | sort)

        # include 路径同样转换为本地工具链可识别的形式（Windows: D:/a/...）
        RAPIDJSON_INCLUDE_BP="$(to_build_path "${RAPIDJSON_INCLUDE}")"
        SHIM_DIR_BP="$(to_build_path "${SHIM_DIR}")"
...
fn main() {
    ...
    cc_build.include("${RAPIDJSON_INCLUDE_BP}");
    cc_build.include("${SHIM_DIR_BP}");
    // ② 编译 shim 的 *_ffi.cpp，提供 rapidjson_* extern "C" 实现符号
${SHIM_FILE_LINES}
