#!/usr/bin/env pwsh
# =============================================================================
# verify-tinyxml2-ffi.ps1  (Windows PowerShell)
#
# 用途：本地验证 cpp2rust-demo 对 tinyxml2 C++ 库的完整 Rust safe FFI 生成能力
#
# 流程总览：
#   § 0. 环境检查
#   § 1. 安装 cpp2rust-demo
#   § 2. 定位本地 tinyxml2 源码（references/tinyxml2）
...
#   § 7. 生成结果汇报
#
# 说明：tinyxml2 为 hicc-direct（无 shim）场景，直接以 tinyxml2.cpp 为输入，重点检查
# build.rs 是否注入头路径与库实现，以及 import_class! 是否成功生成。
# =============================================================================
...
# ─── 可配置参数 ───────────────────────────────────────────────────────────────
$Feature     = if ($env:FEATURE) { $env:FEATURE } else { "tinyxml2_ffi" }
...
try {
    ...
    Step "§ 2. 定位本地 tinyxml2 源码"
    $LibDir = Join-Path $RepoDir 'references/tinyxml2'
    $SourceFile = Join-Path $LibDir 'tinyxml2.cpp'
    if (-not (Test-Path $SourceFile)) { Fail 'tinyxml2 子模块未初始化，请运行：git submodule update --init references/tinyxml2' }
    Info "库目录：$LibDir"
    Info "源文件：$SourceFile"
    Info "include 目录：$LibDir"
    Ok 'tinyxml2 源码就绪'

    Step "§ 3. 编译库目标文件（供 nm 符号验证）"
    $ObjDir = Join-Path $RuntimeDir 'obj'
    $null = New-Item -ItemType Directory -Force -Path $ObjDir
    $ObjFile = Join-Path $ObjDir 'tinyxml2.obj'
    $CompileLog = Join-Path $ObjDir 'tinyxml2.log'
    $compileExit = Invoke-CxxCompile -Compiler $Compiler -Std $CxxStd -IncludeDirs @($LibDir) -Sources @($SourceFile) -Output $ObjFile -LogFile $CompileLog
    if ($compileExit -eq 0) { Ok '编译成功：tinyxml2.obj' } else { Warn '编译失败：tinyxml2.cpp'; if (Test-Path $CompileLog) { Get-Content $CompileLog | Write-Host } }
    $ObjectFiles = @($ObjFile)

    Step "§ 4. cpp2rust-demo init（捕获 FFI 脚手架）"
    $BuildScript = Join-Path $RuntimeDir 'build-tinyxml2.ps1'
    Write-CompileBuildScript -Path $BuildScript -Compiler $Compiler -Std $CxxStd -IncludeDirs @($LibDir) -SourceFiles @($SourceFile) -OutputDir (Join-Path $RuntimeDir 'build')
    ...
    if (Test-Path $BuildRs) {
        ...
        if ($BuildRsText -match 'cc_build\.include' -and $BuildRsText -match 'cc_build\.file') { Ok 'build.rs 已由工具自动注入头路径 + 库实现（方案 A 生效）' }
        else {
            Warn '工具 build.rs 未注入头/实现路径，退回脚本就地补全（方案 B 兜底）'
            Write-FallbackBuildRs -BuildRs $BuildRs -RustProject $RustProject -LibName $LibName -Std $CxxStd -IncludeDirs @($LibDir) -SourceFiles @($SourceFile) -CommentLines @('与库源码编译保持一致的 C++ 标准','① tinyxml2 头文件包含路径（hicc 生成的胶水 C++ #include 需要）','② 编译 tinyxml2.cpp，提供底层 C++ 实现符号') -LinkStdCpp
    ...
    Step "§ 6. FFI 验证"
    Write-Host "`n6a. 库目标文件核心 C++ 符号（nm 验证）"
    $NmOutput = Get-DefinedSymbols -NmTool $NmTool -ObjectFiles $ObjectFiles
    $MatchedSymbols = @($NmOutput | Where-Object { $_ -match 'XMLDocument|XMLElement' })
    Info "库目标文件已定义符号数：$($NmOutput.Count)"
    Info "匹配 XMLDocument/XMLElement 的符号数：$($MatchedSymbols.Count)"
    if ($MatchedSymbols.Count -gt 0) { $MatchedSymbols | Select-Object -First 20 | ForEach-Object { Write-Host "  $_" }; Ok '目标文件中存在 XMLDocument/XMLElement 相关符号' } else { Warn '未在 tinyxml2.obj 中找到 XMLDocument/XMLElement 相关符号'; $script:ScriptErrors++ }

    Write-Host "`n6b. 生成 Rust 代码中的 FFI 声明（import_class! / import_lib!）"
    ...
    if ($ImportClassFiles -gt 0) { Ok '生成代码包含 import_class! 块（类绑定存在）✓' }
    elseif ($ImportLibFiles -gt 0) { Warn '仅检测到 import_lib!，未检测到 import_class!（对于类库这通常不符合预期）'; $script:ScriptErrors++ }
    else { Warn '生成代码中同时未找到 import_class! / import_lib! 块'; $script:ScriptErrors++ }

    Write-Host "`n6c. hicc 直出模式说明（无 shim 函数名交叉比对）"
    Info 'tinyxml2 采用 hicc 直出模式（无 extern-C shim），不适用 shim 函数名交叉比对。'
    ...
    Step "§ 7. 生成结果汇报"
    Write-Host ''
    Write-Host '  项目：      tinyxml2（hicc 直出，无 shim）'
    ...
    if ($ImportClassFiles -gt 0) { Ok '已生成 tinyxml2 Rust safe FFI 脚手架' } else { Warn '未检测到 import_class! 绑定'; $script:ScriptErrors++ }
