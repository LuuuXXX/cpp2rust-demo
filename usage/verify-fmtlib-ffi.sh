...
#
# 用途：本地验证 cpp2rust-demo 对 {fmt} 格式化库的多文件 FFI 生成能力
#
# 背景说明：
#   {fmt} 是现代 C++ 格式化库，包含多个 .cc 源文件（format.cc、os.cc），包含 format_string/formatter 模板类。本脚本验证工具对多翻译单元和非标准扩展名（.cc）的处理能力。
#
# 流程总览：
#   § 0. 环境检查
#   § 1. 安装 cpp2rust-demo
...
#   ① 多翻译单元捕获 —— 工具可同时处理 format.cc / os.cc
#   ② .cc 扩展名兼容 —— 非标准 .cc 源文件可被正常拦截与解析
#   ③ import_class! / hicc::cpp! 输出 —— 生成必要的绑定块
#   ④ 多文件 build.rs 回填 —— fallback 可同时编译多个 .cc 实现
...
SOURCE_DIR="${REPO_DIR}/references/fmtlib"
FMT_INCLUDE="${SOURCE_DIR}/include"
FMT_SRC="${SOURCE_DIR}/src"
FORMAT_CC="${FMT_SRC}/format.cc"
OS_CC="${FMT_SRC}/os.cc"

if [ ! -d "${SOURCE_DIR}" ] || [ ! -f "${FORMAT_CC}" ] || [ ! -f "${OS_CC}" ]; then
    info "references/fmtlib 子模块未初始化，正在初始化..."
    git -C "${REPO_DIR}" submodule update --init references/fmtlib || \
        fail "子模块初始化失败：references/fmtlib；请检查网络或手动执行 git submodule update --init references/fmtlib"
fi

if [ ! -d "${SOURCE_DIR}" ] || [ ! -f "${FORMAT_CC}" ] || [ ! -f "${OS_CC}" ]; then
    fail "未找到 fmtlib 源文件：${FORMAT_CC} / ${OS_CC}"
fi

info "fmt include：${FMT_INCLUDE}"
info "fmt src：${FMT_SRC}"
info "多翻译单元：${FORMAT_CC} ; ${OS_CC}"
ok "fmtlib 源文件就绪"
...
COMPILE_ERRORS=0
for src in "${FORMAT_CC}" "${OS_CC}"; do
    name="$(basename "${src}" .cc)"
    obj="${OBJ_DIR}/${name}.o"
    if g++ -c -std="${CXX_STD}" -I"${FMT_INCLUDE}" -DFMT_HEADER_ONLY=0 "${src}" -o "${obj}" 2>&1; then
        info "编译成功：${name}.o"
    else
...
if [ "${COMPILE_ERRORS}" -gt 0 ]; then
    warn "${COMPILE_ERRORS} 个 fmtlib 源文件编译失败，nm 验证可能不完整"
else
    ok "fmtlib 多翻译单元编译完成"
fi
...
set -e
g++ -c -std="${CXX_STD}" -I"${FMT_INCLUDE}" -DFMT_HEADER_ONLY=0 "${FORMAT_CC}" -o /dev/null 2>&1
g++ -c -std="${CXX_STD}" -I"${FMT_INCLUDE}" -DFMT_HEADER_ONLY=0 "${OS_CC}" -o /dev/null 2>&1
EOF
...
"
        ...
        FORMAT_CC_BP="$(to_build_path "${FORMAT_CC}")"
        OS_CC_BP="$(to_build_path "${OS_CC}")"
        cat > "${BUILD_RS}" << EOF
fn main() {
...
# ── 6a. 多翻译单元目标文件符号（nm） ─────────────────────────────────────────
echo -e "\n${BOLD}6a. 多翻译单元目标文件符号（nm）${NC}"
NM_CACHE=$(mktemp)
find "${OBJ_DIR}" -name "*.o" 2>/dev/null \
    | xargs -r nm -C --defined-only 2>/dev/null > "${NM_CACHE}" || true

UNIT_COUNT=$(find "${OBJ_DIR}" -name "*.o" 2>/dev/null | wc -l)
FMT_SYMBOLS=$(grep -c ' [TWBV] ' "${NM_CACHE}" 2>/dev/null || echo 0)
info "已编译目标文件数：${UNIT_COUNT}"
info "目标文件中已定义符号数：${FMT_SYMBOLS}"
if [ "${UNIT_COUNT}" -ge 2 ]; then
    ok "已处理 format.cc / os.cc 两个翻译单元"
else
    warn "目标文件数不足 2（多翻译单元验证不完整）"
fi
if [ "${FMT_SYMBOLS}" -gt 0 ]; then
...
if [ -d "${RUST_SRC}" ]; then
    ...
    if [ "${UNIT_RS_FILES}" -ge 2 ]; then
        ok "生成结果覆盖多个翻译单元（.cc 非标准扩展名处理正常）"
    else
        warn "生成 Rust 文件数小于 2，多文件捕获可能不完整"
    fi

    echo "──── 生成的 Rust 文件（前 20 条）────"
    find -L "${RUST_SRC}" -name "*.rs" | sort | head -20 || true
else
    warn "Rust 源码目录不存在：${RUST_SRC}"
fi

# ── 6c. link_name 一致性 / .cc 多单元交叉比对 ────────────────────────────────
echo -e "\n${BOLD}6c. link_name 一致性 / .cc 多单元交叉比对${NC}"
...
if [ -n "${LINK_NAMES}" ]; then
    ...
    if [ "${BAD_LINKS}" -eq 0 ]; then
        ok "link_name 未受路径污染（.cc 多单元处理正常）"
    else
        warn "${BAD_LINKS} 个 link_name 含路径分隔符"
    fi
else
    warn "未找到 link_name 声明（可能只生成 hicc::cpp! / import_class!）"
fi
...
echo -e "${BOLD}└─────────────────────────────────────────────────────────┘${NC}"
echo ""
echo -e "  ${BOLD}项目：${NC}      {fmt}（多翻译单元 + .cc 扩展名）"
echo -e "  ${BOLD}feature：${NC}   ${FEATURE}"
echo -e "  ${BOLD}源文件：${NC}    ${FORMAT_CC} ; ${OS_CC}"
...
if [ -d "${RUST_SRC}" ]; then
    ...
    if [ "${RS_FILES}" -ge 2 ]; then
        ...
        echo -e "  ${RED}✗ 生成 Rust 文件数不足 2（多文件捕获可能失败）${NC}"
...
SKILL 适用场景（多翻译单元工作流）：
  ...
  在 fmtlib 源文件目录 与 GitHub Copilot 对话，说：
    "帮我用 cpp2rust-demo 把 fmtlib 多个 .cc 源文件转换为 Rust FFI"
  或：
    "对 fmtlib 项目执行 cpp2rust-demo 转换"

────────────────────────────────────────────────────────────────
多翻译单元库的典型工作流（以 {fmt} 为例）：

  1. 确认待分析的多个 .cc / .h 文件（如 format.cc、os.cc）

  2. 在源码目录准备可复现的 build script，并覆盖全部翻译单元

  3. 运行 cpp2rust-demo init
     cpp2rust-demo init --feature fmtlib_ffi -- bash build.sh

  4. 运行 cpp2rust-demo merge
     cpp2rust-demo merge --feature fmtlib_ffi

  5. 在生成的 Rust 项目中使用 import_class! / hicc::cpp! 绑定调用 fmt API

────────────────────────────────────────────────────────────────
⚠  重要提示：
  cpp2rust-demo 处理多翻译单元时，依赖 build script 实际编译每个源文件。
  若遗漏某个 .cc 文件，生成结果可能不完整，只会覆盖部分 API，
  不会体现完整的多文件 FFI 绑定。
