...
#
# 用途：本地验证 cpp2rust-demo 对 fmtlib 现代 C++ 多文件 + CMake 项目的 Rust safe FFI 生成能力
#
# 背景说明：
#   fmtlib 是著名的现代 C++ 格式化库，使用 CMake 构建系统，包含多个源文件，
#   大量使用 C++11/14/17 特性。这是验证 cpp2rust-demo 对 CMake 项目支持的最佳场景。
#
# 流程总览：
#   § 0. 环境检查
#   § 1. 安装 cpp2rust-demo
#   § 2. 定位本地 fmtlib 源文件（references/fmtlib）+ 检测 CMake
#   § 3. CMake 配置 + 构建 fmtlib
#   § 4. cpp2rust-demo init  —— 编译拦截 & Rust FFI 脚手架生成
#   § 5. cpp2rust-demo merge —— 整理输出目录结构
#   § 6. FFI 验证（符号、import_class!、link_name）
#   § 7. 生成结果汇报
#
# 本脚本验证的 cpp2rust-demo 特性：
#   ① CMake 项目处理      —— 标准 CMake 构建流程
#   ② 多文件项目          —— src/*.cc 多源文件
#   ③ 现代 C++ 绑定       —— fmt::format、fmt::print 等
#   ④ 命名空间处理        —— fmt:: 命名空间
#
# 系统要求（Ubuntu/Debian）：
#   sudo apt-get install -y clang libclang-dev g++ libstdc++-14-dev \
#                           binutils git curl cmake
...
# =============================================================================
# § 2. 定位本地 fmtlib 源文件 + 检测 CMake
# =============================================================================
step "§ 2. 定位本地 fmtlib 源文件"
...
FMTLIB_DIR="${REPO_DIR}/references/fmtlib"

# 验证 fmtlib 目录存在
if [ ! -d "${FMTLIB_DIR}" ]; then
    warn "fmtlib 子模块未初始化：${FMTLIB_DIR}"
    warn "请运行：git submodule update --init references/fmtlib"
    info "脚本将跳过验证"
    exit 0
fi

CMAKE_FILE="${FMTLIB_DIR}/CMakeLists.txt"
if [ ! -f "${CMAKE_FILE}" ]; then
    fail "未找到 CMakeLists.txt：${CMAKE_FILE}"
fi

info "fmtlib 目录：${FMTLIB_DIR}"
info "CMake 文件：${CMAKE_FILE}"
...
# § 3. CMake 配置 + 构建 fmtlib
# =============================================================================
step "§ 3. CMake 配置 + 构建 fmtlib"

BUILD_DIR="${FMTLIB_DIR}/build"
info "CMake 构建目录：${BUILD_DIR}"

# 注册 trap 清理临时目录
trap 'rm -rf "${NM_CACHE:-}" 2>/dev/null || true' EXIT

# CMake 配置
if [ ! -d "${BUILD_DIR}" ]; then
    info "执行 CMake 配置..."
    if cmake -S "${FMTLIB_DIR}" -B "${BUILD_DIR}" \
            -DCMAKE_BUILD_TYPE=Release \
            -DFMT_DOC=OFF \
            -DFMT_TEST=OFF 2>&1; then
        ok "CMake 配置成功"
    else
        warn "CMake 配置失败"
...
# 查找生成的库文件
FMT_LIB=$(find "${BUILD_DIR}" -name "libfmt.a" -o -name "libfmt.so" 2>/dev/null | head -1)
if [ -n "${FMT_LIB}" ]; then
    info "找到 fmtlib 库：${FMT_LIB}"
    ok "fmtlib 构建产物就绪"
else
    warn "未找到 libfmt 库文件"
fi
...
# 创建临时构建脚本：CMake 构建，触发 LD_PRELOAD hook 拦截
BUILD_SCRIPT=$(mktemp)
cat > "${BUILD_SCRIPT}" << EOF
#!/bin/bash
# 临时构建脚本：由 cpp2rust-demo init 的 LD_PRELOAD hook 拦截编译调用
set -e
# 清理并重新构建
rm -rf "${BUILD_DIR}"
cmake -S "${FMTLIB_DIR}" -B "${BUILD_DIR}" \\
      -DCMAKE_BUILD_TYPE=Release \\
      -DFMT_DOC=OFF \\
      -DFMT_TEST=OFF 2>&1
...
"
        ...
        # fmtlib 源文件路径转换（Windows 兼容）
        FMTLIB_DIR_BP="$(to_build_path "${FMTLIB_DIR}")"

        # 收集 fmtlib 源文件
        FMT_SOURCES=""
        for src in "${FMTLIB_DIR}"/src/*.cc; do
            if [ -f "${src}" ]; then
                src_bp="$(to_build_path "${src}")"
                FMT_SOURCES="${FMT_SOURCES}    cc_build.file(\"${src_bp}\");
"
            fi
        done
...
fn main() {
    ...
    // ① fmtlib 头文件包含路径
    cc_build.include("${FMTLIB_DIR_BP}/include");
    // ② 编译 fmtlib 源文件
${FMT_SOURCES}    // 逐文件注册含 hicc 宏的单元
    build
...
# ── 6a. 验证 libfmt 中的符号 ──────────────────────────────────────────────────
echo -e "\n${BOLD}6a. libfmt 符号（nm 验证）${NC}"
NM_CACHE=$(mktemp)
if [ -n "${FMT_LIB}" ] && [ -f "${FMT_LIB}" ]; then
    nm --defined-only -f posix "${FMT_LIB}" 2>/dev/null > "${NM_CACHE}" || true
    SYMBOL_COUNT=$(grep -c ' T ' "${NM_CACHE}" 2>/dev/null || echo 0)
    info "libfmt 中 T 段定义符号数：${SYMBOL_COUNT}"

    if [ "${SYMBOL_COUNT}" -gt 0 ]; then
        echo "──── 部分 fmt 符号（前 30 条，T 段）────"
        awk '$2 == "T" { print $1 }' "${NM_CACHE}" | grep -i 'fmt' | head -30 || true
        ok "libfmt 包含 fmt 符号"
    else
        warn "未找到 T 段符号（libfmt 是否构建成功？）"
    fi
else
    warn "未找到 libfmt，跳过符号验证"
fi
...
if [ -d "${RUST_SRC}" ]; then
    ...
    if [ "${IMPORT_CLASS_FILES}" -gt 0 ] || [ "${IMPORT_LIB_FILES}" -gt 0 ]; then
        ok "生成代码包含 FFI 绑定 ✓"
        echo ""
        echo "──── FFI 声明（前 30 条）────"
        grep -rn "class \|pub fn " "${RUST_SRC}" 2>/dev/null | grep -v "//\|#\[" | head -30 || true
    else
        warn "生成代码中未找到 FFI 绑定"
    fi
else
...
echo -e "${BOLD}└─────────────────────────────────────────────────────────┘${NC}"
echo ""
echo -e "  ${BOLD}项目：${NC}      fmtlib（现代 C++ 多文件 + CMake）"
echo -e "  ${BOLD}feature：${NC}   ${FEATURE}"
echo -e "  ${BOLD}源目录：${NC}    ${FMTLIB_DIR}"
...
if [ -d "${RUST_SRC}" ]; then
    IMPORT_CLASS_FILES=$(grep -rl "hicc::import_class!" "${RUST_SRC}" 2>/dev/null | wc -l)
    IMPORT_LIB_FILES=$(grep -rl "hicc::import_lib!" "${RUST_SRC}" 2>/dev/null | wc -l)
    TOTAL_BINDINGS=$((IMPORT_CLASS_FILES + IMPORT_LIB_FILES))
    echo -e "  ${BOLD}FFI 绑定文件数：${NC}  ${TOTAL_BINDINGS} (class: ${IMPORT_CLASS_FILES}, lib: ${IMPORT_LIB_FILES})"
    if [ "${TOTAL_BINDINGS}" -gt 0 ]; then
        echo -e "  ${GREEN}✓ 成功生成 Rust safe FFI${NC}"
    else
        echo -e "  ${RED}✗ 未生成 FFI 绑定${NC}"
        SCRIPT_ERRORS=$((SCRIPT_ERRORS + 1))
    fi
fi
