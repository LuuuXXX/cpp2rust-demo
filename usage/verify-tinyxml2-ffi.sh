...
#
# 用途：验证 cpp2rust-demo 对 tinyxml2 C++ 库的完整 Rust safe FFI 生成能力
#
# 背景说明：
#   tinyxml2 是 C++ 库（无 shim 层），cpp2rust-demo 采用 hicc 直出（无 shim）模式，
#   直接以库源码编译拦截结果生成 Rust safe FFI。本脚本以 tinyxml2.cpp 为输入演示
...
#
#   ┌──────────────────────────────────────────────────────────────┐
#   │  纯 C++ 库（如 tinyxml2）的 hicc 直出工作流：            │
#   │                                                              │
#   │  1. 直接编译库源码（tinyxml2.cpp）供 cpp2rust-demo 拦截  │
...
LIB_DIR="${REPO_DIR}/references/tinyxml2"
if [ ! -d "${LIB_DIR}" ] || [ ! -f "${LIB_DIR}/tinyxml2.cpp" ]; then
    fail "tinyxml2 子模块未初始化，请运行：git submodule update --init references/tinyxml2"
fi

SOURCE_FILE="${LIB_DIR}/tinyxml2.cpp"

info "库目录：${LIB_DIR}"
info "源文件：${SOURCE_FILE}"
info "include 目录：${LIB_DIR}"
ok "tinyxml2 源码就绪"
...
COMPILE_LOG="${OBJ_DIR}/tinyxml2.log"
COMPILE_ERRORS=0
if g++ -c -std="${CXX_STD}" -I"${LIB_DIR}" "${LIB_DIR}/tinyxml2.cpp" -o "${OBJ_FILE}" >"${COMPILE_LOG}" 2>&1; then
    ok "编译成功：tinyxml2.o"
else
...
g++ -c -std="${CXX_STD}" \
    -I"${LIB_DIR}" \
    "${LIB_DIR}/tinyxml2.cpp" -o /dev/null 2>&1
EOF
...
"
                ...
                LIB_DIR_BP="$(to_build_path "${LIB_DIR}")"
...
if [ -f "${OBJ_FILE}" ]; then
    ...
    MATCHED_SYMBOLS=$(grep -E 'XMLDocument|XMLElement' "${NM_CACHE}" 2>/dev/null | wc -l | tr -d '[:space:]')
    info "库目标文件已定义符号数：${DEFINED_SYMBOLS}"
    info "匹配 XMLDocument/XMLElement 的符号数：${MATCHED_SYMBOLS}"

    if [ "${MATCHED_SYMBOLS}" -gt 0 ]; then
        echo "──── 部分核心类符号（前 20 条）────"
        grep -E 'XMLDocument|XMLElement' "${NM_CACHE}" | head -20 || true
        ok "目标文件中存在 XMLDocument/XMLElement 相关符号"
    else
        warn "未在 tinyxml2.o 中找到 XMLDocument/XMLElement 相关符号"
    fi
else
...
echo -e "${BOLD}└─────────────────────────────────────────────────────────┘${NC}"
echo ""
echo -e "  ${BOLD}项目：${NC}      tinyxml2（hicc 直出，无 shim）"
...
────────────────────────────────────────────────────────────────
纯 C++ 库的典型 hicc 直出工作流（以 tinyxml2 为例）：

  1. 直接编译库源码
     参考：references/tinyxml2/tinyxml2.cpp

  2. 在源码目录编写 Makefile 或 build script

  3. 运行 cpp2rust-demo init
     cpp2rust-demo init --feature tinyxml2_ffi -- bash build-tinyxml2.sh

  4. 运行 cpp2rust-demo merge
     cpp2rust-demo merge --feature tinyxml2_ffi

  5. 在生成的 Rust 项目中使用 import_class! 绑定调用 tinyxml2 API

────────────────────────────────────────────────────────────────
⚠  重要提示：
  tinyxml2 采用 hicc 直出模式（无 extern-C shim），不适用 shim 函数名交叉比对。
