...
#
# 背景说明：
#   tinyxml2 是单头文件 + 单 .cpp 的经典 XML 解析库（tinyxml2::命名空间），
#   包含典型 OOP 类层级：XMLDocument → XMLElement → XMLNode，约 4K 行代码。
...
#   § 0. 环境检查
#   § 1. 安装 cpp2rust-demo
#   § 2. 定位 tinyxml2 源码（references/tinyxml2 子模块 或 临时克隆）
#   § 3. 编译 tinyxml2.cpp 目标文件（供后续 nm 符号验证）
...
#
# 可配置环境变量：
#   FEATURE       （默认 tinyxml2）      cpp2rust-demo feature 名称
...
# § 2. 定位 tinyxml2 源码
# =============================================================================
step "§ 2. 定位 tinyxml2 源码"

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_DIR="$(git -C "${SCRIPT_DIR}" rev-parse --show-toplevel 2>/dev/null || echo "${SCRIPT_DIR}/..")"
REPO_DIR="$(cd "${REPO_DIR}" && pwd)"

SUBMODULE_DIR="${REPO_DIR}/references/tinyxml2"
CLONE_DIR="${TMPDIR}/tinyxml2-ffi-demo"

if [ -f "${SUBMODULE_DIR}/tinyxml2.cpp" ]; then
    TINYXML2_DIR="${SUBMODULE_DIR}"
    info "使用仓库子模块：${TINYXML2_DIR}"
else
    info "子模块未初始化，克隆 tinyxml2 到 ${CLONE_DIR}…"
    if [ -d "${CLONE_DIR}" ]; then
        info "目录已存在，尝试 git pull…"
        git -C "${CLONE_DIR}" pull --ff-only 2>/dev/null || true
    else
        git clone --depth 1 https://github.com/leethomason/tinyxml2.git "${CLONE_DIR}"
    fi
    TINYXML2_DIR="${CLONE_DIR}"
fi

if [ ! -f "${TINYXML2_DIR}/tinyxml2.cpp" ]; then
    fail "未找到 tinyxml2.cpp：${TINYXML2_DIR}/tinyxml2.cpp"
fi

info "tinyxml2 源码目录：${TINYXML2_DIR}"
ok "tinyxml2 源码就绪"

# =============================================================================
# § 3. 编译 tinyxml2.cpp 目标文件（供 nm 符号验证）
# =============================================================================
step "§ 3. 编译 tinyxml2.cpp 目标文件"

OBJ_DIR=$(mktemp -d)
trap 'rm -rf "${OBJ_DIR}" "${NM_CACHE:-}" 2>/dev/null || true' EXIT

OBJ_FILE="${OBJ_DIR}/tinyxml2.o"
...
if g++ -c -std="${CXX_STD}" \
    ...
    ok "tinyxml2.cpp 编译成功 → ${OBJ_FILE}"
else
    warn "tinyxml2.cpp 编译失败，nm 验证可能不完整"
    cat "${COMPILE_LOG}" >&2
fi
...
info "构建命令：g++ -c -std=${CXX_STD} -I${TINYXML2_DIR} ${TINYXML2_DIR}/tinyxml2.cpp"
...
"
        ...
        TINYXML2_CPP_BP="$(to_build_path "${TINYXML2_DIR}/tinyxml2.cpp")"
...
EOF
        ...
        sed 's/^/    /' "${BUILD_RS}"
        ok "build.rs 已就地补全（注入头路径 + tinyxml2.cpp + 逐文件注册）"
    fi
else
...
# ── 6a. nm 查看 .o 中的 T 段符号 ─────────────────────────────────────────────
echo -e "\n${BOLD}6a. tinyxml2 目标文件符号（nm 验证，预期 _ZN9tinyxml2* 等）${NC}"
NM_CACHE=$(mktemp)
if [ -f "${OBJ_FILE}" ]; then
...
if [ "${T_COUNT}" -gt 0 ]; then
    ...
    # 检查是否含 tinyxml2 命名空间符号（_ZN9tinyxml2）
    if grep -q '_ZN9tinyxml2' "${NM_CACHE}" 2>/dev/null; then
        ok "检测到 tinyxml2:: 命名空间符号（_ZN9tinyxml2*）✓"
    else
        warn "未检测到 _ZN9tinyxml2* 符号（可能 tinyxml2 命名空间未编译）"
    fi
else
...
if [ -d "${RUST_SRC}" ]; then
    ...
    if [ "${IMPORT_CLASS_FILES}" -gt 0 ]; then
        ...
        # 检查关键类名
        for cls in XMLDocument XMLElement XMLNode; do
        ...
        warn "生成代码中未找到 import_class! 块（tinyxml2 OOP 类未被提取？）"
...
echo -e "${BOLD}└─────────────────────────────────────────────────────────┘${NC}"
echo ""
echo -e "  ${BOLD}项目：${NC}      tinyxml2（idiomatic C++ OOP，无 shim）"
echo -e "  ${BOLD}feature：${NC}   ${FEATURE}"
echo -e "  ${BOLD}源码目录：${NC}  ${TINYXML2_DIR}"
...
if [ -d "${RUST_SRC}" ]; then
    ...
    if [ "${IMPORT_CLASS_FILES}" -gt 0 ]; then
        ...
        echo -e "  ${RED}✗ 未生成 OOP 类绑定（请检查 tinyxml2 是否被正确捕获）${NC}"
        SCRIPT_ERRORS=$((SCRIPT_ERRORS + 1))
    fi
fi
