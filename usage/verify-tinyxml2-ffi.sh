...
#   XMLDocument → XMLElement → XMLNode，代码约 4K 行，复杂度最低，是最简单
#   的端到端验证项目。
#
# 流程总览：
#   § 0. 环境检查
#   § 1. 安装 cpp2rust-demo
#   § 2. 定位本地 tinyxml2 源文件（references/tinyxml2/tinyxml2.cpp）
#   § 3. 编译 tinyxml2.o（供后续 nm 符号验证）
...
#   ① 单文件项目处理 —— 最简单的 E2E 场景
#   ② OOP 类继承     —— XMLNode 基类 / XMLElement 子类
#   ③ 方法绑定生成   —— import_class! 块包含所有公开方法
#   ④ 头文件包含     —— 同目录 #include "tinyxml2.h" 探测
#   ⑤ hicc 直出模式  —— 直接绑定命名空间类型，无需 extern-C shim
#
# 系统要求（Ubuntu/Debian）：
...
TINYXML2_DIR="${REPO_DIR}/references/tinyxml2"

# 验证 tinyxml2 目录存在
if [ ! -d "${TINYXML2_DIR}" ]; then
    warn "tinyxml2 子模块未初始化：${TINYXML2_DIR}"
    warn "请运行：git submodule update --init references/tinyxml2"
    info "脚本将跳过验证"
    exit 0
fi

TINYXML2_CPP="${TINYXML2_DIR}/tinyxml2.cpp"
if [ ! -f "${TINYXML2_CPP}" ]; then
    fail "未找到 tinyxml2.cpp：${TINYXML2_CPP}"
fi

info "tinyxml2 目录：${TINYXML2_DIR}"
info "源文件：${TINYXML2_CPP}"

ok "tinyxml2 源文件就绪"

# =============================================================================
# § 3. 编译 tinyxml2.o（供 nm 符号验证）
# =============================================================================
step "§ 3. 编译 tinyxml2.o"
...
TINYXML2_OBJ="${OBJ_DIR}/tinyxml2.o"
if g++ -c -std="${CXX_STD}" \
       -I"${TINYXML2_DIR}" \
       "${TINYXML2_CPP}" -o "${TINYXML2_OBJ}" 2>&1; then
    ok "编译成功：tinyxml2.o"
else
    warn "编译失败：tinyxml2.cpp"
...
# 创建临时构建脚本：g++ -c 编译 tinyxml2.cpp，触发 LD_PRELOAD hook 拦截
BUILD_SCRIPT=$(mktemp)
...
"
        ...
        # tinyxml2 源文件路径转换（Windows 兼容）
        TINYXML2_CPP_BP="$(to_build_path "${TINYXML2_CPP}")"
        TINYXML2_DIR_BP="$(to_build_path "${TINYXML2_DIR}")"
...
fn main() {
    ...
    // ① tinyxml2 头文件包含路径（hicc 生成的胶水 C++ #include 需要）
    cc_build.include("${TINYXML2_DIR_BP}");
    // ② 编译 tinyxml2.cpp，提供类实现符号
    cc_build.file("${TINYXML2_CPP_BP}");
...
# ── 6a. 验证 tinyxml2.o 中的符号 ──────────────────────────────────────────────
echo -e "\n${BOLD}6a. tinyxml2.o 符号（nm 验证）${NC}"
NM_CACHE=$(mktemp)
if [ -f "${TINYXML2_OBJ}" ]; then
    nm --defined-only -f posix "${TINYXML2_OBJ}" 2>/dev/null > "${NM_CACHE}" || true
    SYMBOL_COUNT=$(grep -c ' T ' "${NM_CACHE}" 2>/dev/null || echo 0)
    info "tinyxml2.o 中 T 段定义符号数：${SYMBOL_COUNT}"

    if [ "${SYMBOL_COUNT}" -gt 0 ]; then
        echo "──── 部分类符号（前 20 条，T 段）────"
        awk '$2 == "T" { print $1 }' "${NM_CACHE}" | head -20 || true
        ok "tinyxml2.o 包含类方法符号"
    else
        warn "未找到 T 段符号（tinyxml2.cpp 是否编译成功？）"
    fi
else
    warn "未找到 tinyxml2.o，跳过符号验证"
fi
...
echo ""
echo -e "${BOLD}┌─────────────────────────────────────────────────────────┐${NC}"
echo -e "${BOLD}│             cpp2rust-demo tinyxml2 FFI 验证结果         │${NC}"
echo -e "${BOLD}└─────────────────────────────────────────────────────────┘${NC}"
echo ""
echo -e "  ${BOLD}项目：${NC}      tinyxml2（单头 + 单 .cpp XML 解析库）"
echo -e "  ${BOLD}feature：${NC}   ${FEATURE}"
echo -e "  ${BOLD}源文件：${NC}    ${TINYXML2_CPP}"
...
if [ -d "${RUST_SRC}" ]; then
    IMPORT_CLASS_FILES=$(grep -rl "hicc::import_class!" "${RUST_SRC}" 2>/dev/null | wc -l || true)
    echo -e "  ${BOLD}import_class! 类绑定文件数：${NC}  ${IMPORT_CLASS_FILES}"
    if [ "${IMPORT_CLASS_FILES}" -gt 0 ]; then
        echo -e "  ${GREEN}✓ 成功生成 Rust safe FFI（import_class! 块存在）${NC}"
    else
        echo -e "  ${YELLOW}⚠ 未生成类绑定（本次驱动可能仅触发自由函数/骨架路径）${NC}"
    fi
fi
