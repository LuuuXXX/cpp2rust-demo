...
struct MergeArgs {
    ...
    ///
    /// When `operator_shims.hpp` was generated by `init`, it is copied into
    /// `<DIR>/meta/` and the exported `build.rs` is rewritten to reference it
    /// via the relative path `"meta"` instead of the original absolute path.
    /// This makes the export self-contained for the shim header.
    ...
    if let Some(output_dir) = &args.output {
        ...
        if lo.meta_dir.join("operator_shims.hpp").exists() {
            ...
            let export_inc: Vec<String> = include_dirs
                ...
                .collect();
            let export_inc_refs: Vec<&str> = export_inc.iter().map(|s| s.as_str()).collect();
            std::fs::write(
                output_abs.join("build.rs"),
                codegen::render_build_rs(&link_name, no_link, &["src/lib.rs"], &export_inc_refs),
            )
            .map_err(|e| anyhow!("rewrite exported build.rs: {}", e))?;
        }
