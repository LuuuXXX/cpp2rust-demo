...
pub fn build_dynamic_casts(ast: &crate::ast_parser::CppAst) -> Vec<DynamicCastSpec> {
    ...
    for ci in &ast.classes {
        ...
        // 遍历直接基类，针对每个「多态基类」生成 基类 → 派生类 的下行转换骨架
        for base in &ci.bases {
            let base_name = clean_type(&base.name).to_string();
            if base_name.is_empty() || base_name == ci.name {
                continue;
            }
            let base_ci = match ast.classes.iter().find(|c| c.name == base_name) {
                Some(c) => c,
                None => continue,
            };
            if !is_polymorphic(base_ci, &ast.classes) {
                continue;
            }
            out.push(build_one(&base_name, &ci.name));
        }
    }
