...
///
/// 生成项目自动引入 `hicc-std` 依赖（Linux / Windows 均支持）。工具生成的 Rust FFI
/// 代码本身不直接依赖 `hicc_std::` 类型，STL 容器均通过 C++ 侧自定义包装类暴露为普通
/// `extern "C"` 接口；`hicc_std::` 类型别名（如 `hicc_std::string`、`hicc_std::vector`
/// 等）可在 Linux / Windows 上直接使用。
...
///
/// 同 [`write_cargo_toml`]，自动引入 `hicc-std` 依赖（Linux / Windows 均支持）。
...
mod tests {
    ...
    fn write_multi_feature_cargo_toml_contains_features() {
        ...
        assert!(
            content.contains("hicc-std = { version = \"0.2\" }"),
            "多 feature Cargo.toml 应引入 hicc-std 依赖"
        );
    }
