...
cat .cpp2rust/sa01/rust/src/entry.rs
```
...
### `entry.rs`（dynamic_cast 注释骨架，⚙️ 末尾追加）
...
打开 `.cpp2rust/sa01/rust/src/entry.rs`，去掉末尾 `dynamic_cast` 注释骨架中目标 cast 对的注释符即可。
...
ClassIR { bases: ["Animal"], ... }
    │  codegen
    ▼
entry.rs          ──────── 正常继承绑定 + dynamic_cast 注释骨架 (✅/⚙️)
    │
...
| codegen | 在 `entry.rs` 末尾追加**注释**骨架 | 无 |
| 解锁 | — | 解注释 `entry.rs` 末尾 `dynamic_cast` 骨架 |
...
| 多重继承 | 当前仅处理完全独立的非虚继承链；多基类 cast 路径可能不完整 |
