hicc::cpp! {
    ...
    std::unique_ptr<Circle> circle_new(double r) {
        return std::make_unique<Circle>(r);
    }
    std::unique_ptr<Rectangle> rectangle_new(double w, double h) {
        return std::make_unique<Rectangle>(w, h);
    }
    std::unique_ptr<Triangle> triangle_new(double b, double h) {
...
hicc::import_lib! {
    ...
    #[cpp(func = "std::unique_ptr<Circle> circle_new(double)")]
    pub fn circle_new_with_r(r: f64) -> Circle;

    #[cpp(func = "std::unique_ptr<Rectangle> rectangle_new(double, double)")]
    pub fn rectangle_new_2(w: f64, h: f64) -> Rectangle;

    #[cpp(func = "std::unique_ptr<Triangle> triangle_new(double, double)")]
