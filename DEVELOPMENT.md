...
**模板绑定生成（v6 Phase B）**：`extractor` 由 `CppAst::template_classes` / `template_functions` 构建 `TemplateClassSpec` / `TemplateFnSpec`（见 `src/extractor/template_spec.rs`），`hicc_codegen` 据此生成泛型 `import_class!`（`pub class Name<T> { ... }`，`#[cpp(class = "template<class T> Name<T>")]` 采用 hicc 要求的完整模板形式）与 `import_lib!` 模板函数骨架。该生成由环境变量 `CPP2RUST_GEN_TEMPLATES` 控制，**默认关闭**：未设置或为 `0`/`false`/`no`/`off` 时不生成模板块，从而保证默认产物逐字节不变（L1 黄金 / L2 编译基线零变更）；设为 `1`/`true`/`on` 时启用。AST 层将 `template_classes` / `template_functions` 的采集口径放宽到**用户代码**（含用户头文件，排除系统头），使头文件中定义的模板（如示例 025 的 `Stack<T>`）可被消费；该放宽仅影响这两个 Phase B 专用字段，不触动 `template_class_ranges`（仍仅限 `.cpp` 自身）与任何默认产物。集成测试见 `tests/template_gen_tests.rs`（`full-test` 门禁）：`cargo test --test template_gen_tests --features full-test -- --test-threads=1`。

**模板实例化别名生成（v6 Phase B 续）**：在泛型骨架之上，`extractor` 进一步扫描用户代码中出现的类型用法（类字段、方法/函数签名，见 `extractor::collect_type_usages`），发现模板类的具体实例化（如示例 025 中字段 `Stack<int>` / `Stack<double>`），由 `template_spec::collect_instantiations` 构建 `TemplateInstantiation`（别名 + Rust 实例化目标 + C++ 实例化目标）。`hicc_codegen` 在 `import_lib!` 中据此生成 `class StackInt = Stack<hicc::Pod<i32>>;` 形式的实例化别名（参照 `references/hicc` 的 hicc-std 写法）。实参映射规则：POD 标量 → `hicc::Pod<rust_scalar>`（如 `int`→`hicc::Pod<i32>`）、已导出 C++ 类 → 直接用类名；含指针/引用或未导出复合类型的实参不可映射，整个实例化被跳过。该能力同样受 `CPP2RUST_GEN_TEMPLATES` 控制、**默认关闭**，默认产物逐字节不变；实例化发现本身仅为数据收集，不影响任何默认产物。

**模板实例化默认构造函数绑定（v6 Phase B 续²）**：在实例化别名之上，`template_spec::template_has_default_ctor` 判定模板类是否具备可访问的默认构造函数（零参 public 构造函数，或无任何用户声明构造函数的非抽象类的隐式默认构造函数；抽象类与仅带参/非 public 构造函数判为 `false`），结果存入 `TemplateClassSpec::has_default_ctor`。当为真时，`hicc_codegen` 为每个实例化别名追加 `make_unique` 形式的默认构造函数绑定：`#[cpp(func = "std::unique_ptr<Stack<int>> hicc::make_unique<Stack<int>>()")]` + `#[member(class = StackInt, method = new)]` + `pub fn stack_int_new() -> StackInt;`（C++ 目标形式取自 `TemplateInstantiation::cpp_target`），并将该类别名注释由 `cpp2rust-todo[TPL]` 升级为普通说明；不具备默认构造函数时仍以 `cpp2rust-todo[TPL]` 提示手动补充构造函数/工厂。仍受 `CPP2RUST_GEN_TEMPLATES` 控制、**默认关闭**，默认产物逐字节不变。
