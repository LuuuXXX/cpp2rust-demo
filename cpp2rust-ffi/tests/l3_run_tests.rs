...
use common::{cargo_run, parse_readme_run_result, normalize_output};
...
macro_rules! run_test {
    ($name:ident, $example:literal) => {
        ...
        fn $name() {
            ...
            let expected = parse_readme_run_result($example);
            assert_eq!(
                normalize_output(output.trim()),
                normalize_output(expected.trim()),
