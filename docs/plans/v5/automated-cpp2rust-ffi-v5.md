...
**cpp2rust-demo 与 c2rust-demo 的关系**：
...
# 示例
cpp2rust-demo init -- make
cpp2rust-demo init --feature core_lib -- make -j4
cpp2rust-demo init -- g++ -c foo.cpp -I.
```
...
cpp2rust-demo merge
cpp2rust-demo merge --feature core_lib
```
...
> cpp2rust-demo 有意采用 `CPP2RUST_*` 和 `.cpp2rust/<feature>` 以避免与系统上其他 c2rust 工具产生目录/变量冲突。
...
| 需要完整构建环境 | 解析失败直接报错退出，错误信息包含文件路径和耗时 |
