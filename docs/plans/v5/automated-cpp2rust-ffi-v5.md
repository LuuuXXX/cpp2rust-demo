...
| 文件 | 参考文档 | 用途 |
|------|----------|------|
| `hook/hook.cpp`（新建） | `docs/references/c2rust-demo.md` §Hook 机制 | 预处理逻辑（C++ 版本，需从头编写） |
| `src/capture.rs`（新建） | `docs/references/c2rust-demo.md` §构建捕获阶段 | LD_PRELOAD 执行逻辑（需从头编写） |

**注意**：`references/c2rust-demo/` 目录当前为空，上述文件均需参考 `docs/references/c2rust-demo.md` 的功能描述从头实现。不存在可直接复用的源文件。
...
```bash
# Step 0: 准备 Hook 文件
# 新建 hook/hook.cpp（C++ 编译器拦截，参考 docs/references/c2rust-demo.md §Hook 机制）
# 新建 src/capture.rs（LD_PRELOAD 执行逻辑，参考 docs/references/c2rust-demo.md §构建捕获阶段）
...
1. [ ] 创建 `hook/hook.cpp`，参考 `docs/references/c2rust-demo.md` §Hook 机制，实现以下功能：
   - C++ 编译器检测（basename 匹配）
   - 预处理捕获逻辑
2. [ ] 修改编译器列表：`gcc, clang, cc` → `g++, clang++, c++`
3. [ ] 修改文件扩展名支持：`.c` → `.cpp, .cc, .cxx, .c++, .C, .cp`
4. [ ] 输出预处理文件（`.c2rust`），使用 `-E -C -P`
5. [ ] 新建 `src/capture.rs`（LD_PRELOAD 执行逻辑），参考 `docs/references/c2rust-demo.md` §构建捕获阶段
6. [ ] 复制 `hook/Makefile` 并适配，添加 `build.rs` 入口调用 `cc` crate 编译 `hook.cpp`
