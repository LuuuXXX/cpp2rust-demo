...
```bash
# Step 0: 准备 Hook 文件
# 创建 hook/hook.cpp（参考 references/c2rust-demo/hook/hook.c，改为 C++ 编译器）
# 复制并适配 capture.rs
cp ../cpp2rust-demo/references/c2rust-demo/src/capture.rs ./src/

# Step 1: 编译拦截（捕获预处理文件）
cd cpp-project/
CPP2RUST_FEATURE_ROOT=.cpp2rust/core_lib \   # <-- .cpp2rust/<feature> 完整路径
...
cpp2rust-ffi init \
    -i .cpp2rust/core_lib \              # 与 CPP2RUST_FEATURE_ROOT 一致
    -o ./rust_hicc \
    --feature core_lib          # 可选；省略则使用 default
...
| 参数 | 必填 | 说明 |
|------|------|------|
| `-i / --input` | ✅ | `CPP2RUST_FEATURE_ROOT` 路径，即 `.cpp2rust/<feature>` |
...
| 变量 | 必填 | 说明 |
|------|------|------|
| `CPP2RUST_FEATURE_ROOT` | ✅ | 特征根目录完整路径（含 feature 名），如 `.cpp2rust/core_lib` |
| `CPP2RUST_PROJECT_ROOT` | ✅ | C++ 项目根目录 |
| `CPP2RUST_CXX` | ❌ | C++ 编译器，默认检测 g++/clang++/c++ |
| `CPP2RUST_DEBUG` | ❌ | 设置为 1 输出调试日志 |

> **与 c2rust-demo 的命名区别**：c2rust-demo 使用 `C2RUST_FEATURE_ROOT` 和 `.c2rust/<feature>` 目录；
> cpp2rust 有意采用 `CPP2RUST_*` 和 `.cpp2rust/<feature>` 以避免与系统上其他 c2rust 工具产生目录/变量冲突。
...
```bash
# 捕获阶段
CPP2RUST_FEATURE_ROOT=.cpp2rust/core_lib make ...
# 产物在 .cpp2rust/core_lib/c/src/*.cpp2rust

# 省略 --feature 时默认使用 default
CPP2RUST_FEATURE_ROOT=.cpp2rust/default make ...
```
...
> 两路信息合并可进一步减少 shim 的生成数量。
>
> **⚠️ 此功能为 P1 可选扩展**：v5 主流程基于 AST 分析即可覆盖大多数场景，
> 符号提取作为后续优化手段，在 v5 初期不实现。
...
| Phase 0 | Hook 机制（hook.cpp，输出 `.cpp2rust`） | P0 | Phase T |
| Phase 0a | 符号提取器（`nm --demangle` → `SymbolTable`） | **P1（可选扩展）** | Phase 0 |
...
# 应在 .cpp2rust/<feature>/c/ 下产出 hello_world.cpp.cpp2rust
```

---

### 9.4a Phase 0a - 符号提取器（P1 可选扩展）

> **⚠️ 此 Phase 为 P1 可选扩展**，v5 初期不实现。参见 §5a。

**任务**（P1 实现时）：
1. [ ] hook.cpp 额外保存 `.o` 文件到 `obj/` 子目录
2. [ ] 实现 `symbol_extractor.rs`：调用 `nm --demangle -C` 解析 `.o` 文件
3. [ ] 分类符号：全局函数 / 成员方法 / 构造函数 / 析构函数 / 运算符 / 静态成员
4. [ ] 输出结构化的 `SymbolTable`，供代码生成器使用
...
| C++ AST 节点遗漏 | 功能缺失 | 扩展解析逻辑；符号提取（Phase 0a P1）可作为补充 |
