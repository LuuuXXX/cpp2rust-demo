...
# 验证 merge 产物
test -f "${OUT}/rust/src/lib.rs"               && echo "[OK] lib.rs (consolidated FFI entry)"
...
# lib.rs（merge 后全局视图）应包含 import_lib! 和 link_name
grep -q "import_lib!"         "${OUT}/rust/src/lib.rs" && echo "[OK] import_lib!"
grep -q 'link_name = "rapidjson"' "${OUT}/rust/src/lib.rs" && echo "[OK] link_name"
grep -q '#include "entry.cpp"' "${OUT}/rust/src/lib.rs" && echo "[OK] include entry.cpp"
