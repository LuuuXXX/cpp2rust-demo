...
> v6 在 v5 基础上新增了四类可选高级映射能力，均通过环境变量开关控制，**默认关闭时产物逐字节不变**。
> 本节说明各能力的触发条件、生成内容与使用方式；详细速查表见 [`docs/references/hicc.md`](references/hicc.md)
> 的"v6 新增能力速查（Phase A–C）"节。
...
1. **泛型 `import_class!` 骨架**：模板类映射为保留泛型 `T` 的 `pub class Foo<T>`，成员方法签名保留泛型参数。骨架带 `// cpp2rust-todo[TMPL]` 占位注释，提示用户按实际实例化类型校验签名。

2. **实例化别名**：工具扫描编译单元中以具体类型实例化该模板类的使用点（字段类型、方法参数/返回类型、全局函数签名、显式实例化、局部变量声明），为每种实例化类型生成类型别名：
   - POD 标量（`int`、`double` 等）用 `hicc::Pod<...>` 包装
   - 非 POD 类型直接使用类名

3. **构造工厂骨架**：从模板类的公有构造函数派生实例化类型的工厂函数，在 `import_lib!` 块中输出。
...
**启用方式**：`CPP2RUST_GEN_TEMPLATES=1 cpp2rust-demo init ...`

### 模板函数泛型骨架（`CPP2RUST_GEN_TEMPLATES=1`）

**触发条件**：C++ 代码中存在 `template<typename T> void foo(T*, T*)` 形式的函数模板声明。

**生成内容**：在 `import_lib!` 中输出泛型占位骨架（`<T>` 为占位符，需替换为实际实例化类型）：
...
**注意**：模板函数骨架需要用户手动将 `<T>` 替换为具体类型（如 `do_swap<int>(int*, int*)`），并添加多个版本（每种实例化类型一个绑定）。

### `@make_proxy` 代理工厂（`CPP2RUST_GEN_PROXY=1`）

**触发条件**：检测到某个具体类（有公有构造函数）直接继承了至少一个纯虚接口类。

**生成内容**：为该类生成 `@make_proxy` 代理工厂骨架，使 Rust 侧可以实现 C++ 抽象类：
...
**启用方式**：`CPP2RUST_GEN_PROXY=1 cpp2rust-demo init ...`
...
**启用方式**：`CPP2RUST_GEN_DYNAMIC_CAST=1 cpp2rust-demo init ...`
...
**关闭方式**：`CPP2RUST_GEN_SMOKE=0 cpp2rust-demo init ...`
...
| 环境变量 | 默认值 | 说明 |
|---------|--------|------|
| `CPP2RUST_GEN_TEMPLATES` | **关闭** | 生成模板类/函数泛型骨架与实例化别名（设为 `1`/`true`/`yes`/`on` 开启） |
| `CPP2RUST_GEN_PROXY` | **关闭** | 生成 `@make_proxy` 代理工厂骨架（设为 `1`/`true`/`yes`/`on` 开启） |
| `CPP2RUST_GEN_DYNAMIC_CAST` | **关闭** | 生成 `@dynamic_cast` 下行转换骨架（设为 `1`/`true`/`yes`/`on` 开启） |
| `CPP2RUST_GEN_SMOKE` | **开启** | 生成 `tests/smoke.rs` 冒烟测试（设为 `0`/`false`/`no`/`off` 关闭） |
...
| C++ 特性 | 原因 | 处理方式 |
|----------|------|---------|
| 模板声明本身 | C ABI 没有泛型 | 只导出实例化结果；v6 可选：`CPP2RUST_GEN_TEMPLATES=1` 生成泛型骨架（需手动补全） |
