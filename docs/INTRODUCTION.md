...
- 对工厂函数、全局函数等生成 `// cpp2rust-todo[SMOKE]` 占位注释，提示用户补充行为断言
...
| **L_smoke（v6 新增）** | 生成的 FFI 绑定行为是否正确 | 对 14 个迁移示例（015-018、023-027、034-038）运行 `cargo test --test smoke`，验证 ABI 与基本行为 |
