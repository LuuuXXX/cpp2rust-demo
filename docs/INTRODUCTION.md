...
```bash
# 解析单个文件（调试用）
cpp2rust-demo parse path/to/foo.cpp2rust
...
当前 L1 测试通过率：**45 / 49（92%）**。4 个失败项都是生成格式与手写参考未对齐，不代表功能完全不可用。

### 当前未完成项

| 项目 | 状态 |
|------|------|
| 菱形继承（018） | shim 生成逻辑待修复 |
| 运算符重载降级格式（019） | 输出格式与 golden 未对齐 |
| 有状态 Lambda（039） | class wrapper 格式未对齐 |
| `std::function`（040） | class wrapper 格式未对齐 |
| `merge` 命令 | 占位提示，实际功能待实现 |
| 模板跨翻译单元合并 | 当前每个文件独立解析 |
| Windows 支持 | 当前仅 Linux LD_PRELOAD |
