...
| Interactive file selection | ✅ `.c2rust` files | ✅ `.cpp2rust` middleware files |
...
Real build command (`init -- ...`)
    │
    ▼  LD_PRELOAD hook capture + preprocess
Captured middleware set (`cpp/**/*.cpp2rust`)
    │
    ▼  Interactive file selection
    │  (auto-selects all in non-TTY; saves selected_files.json)
Selected middleware files
    │
    ▼  Infer project headers from preprocessor line markers
Inferred headers
    │
    ▼  clang -ast-dump=json
...
│   ├── selected_files.json        ← middleware files selected by the user
...
├── cpp/
│   └── <source>.cpp2rust        ← preprocessed middleware generated by hook
