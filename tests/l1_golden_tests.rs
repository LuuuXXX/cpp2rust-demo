mod common;
use common::{golden_file, normalize_content, run_tool_on_example};

macro_rules! golden_test {
    ($name:ident, $example:literal) => {
        #[test]
        fn $name() {
            match run_tool_on_example($example) {
                Err(e) => {
                    panic!(
                ...
                Ok(generated) => {
                    let golden_path = golden_file($example);
                    let golden = std::fs::read_to_string(&golden_path).unwrap_or_else(|e| {
                        panic!("Cannot read golden file {}: {}", golden_path.display(), e)
                    });

                    let gen_norm = normalize_content(&generated);
                    let gold_norm = normalize_content(&golden);

                    assert_eq!(
                        gen_norm, gold_norm,
                        "Golden file mismatch for {}.\nGenerated:\n{}\n\nExpected:\n{}",
                        $example, gen_norm, gold_norm
                    );
                }
            }
        }
