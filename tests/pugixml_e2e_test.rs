...
fn pugixml_merge_phase() {
        ...
        .output()
    {
        ...
        Ok(_) => println!(
            "pugixml_merge_phase: cargo check 通过 ({} 个 unit)",
            unit_paths.len()
        ),
        Err(e) => eprintln!(
            "pugixml_merge_phase: cargo check 跳过（cargo 不可用: {}）",
            e
        ),
    }
    ...
    if smoke_test_path.exists() {
            ...
            .output()
        {
            Ok(output) if !output.status.success() => {
                let stderr = String::from_utf8_lossy(&output.stderr);
                panic!(
                    "pugixml_merge_phase: cargo test 失败（生成的冒烟测试未通过）:\n{}",
                    stderr
                );
            }
            Ok(_) => println!(
                "pugixml_merge_phase: cargo test 通过（生成的冒烟测试全部通过）"
            ),
            Err(e) => eprintln!(
                "pugixml_merge_phase: cargo test 跳过（cargo 不可用: {}）",
                e
            ),
        }
    } else {
        println!("pugixml_merge_phase: cargo test 跳过（未生成 tests/smoke.rs）");
    }
}
