mod common;
use common::{cargo_run, parse_readme_run_result, rust_hicc_dir, workspace_root};
...
macro_rules! run_test {
    ($name:ident, $example:literal) => {
        ...
        fn $name() {
            ...
            let actual = cargo_run(&dir).unwrap_or_else(|e| panic!("{}", e));

            if let Some(expected) = parse_readme_run_result(&readme) {
                let actual_trim = actual.trim();
                let expected_trim = expected.trim();
                assert_eq!(
                    actual_trim, expected_trim,
                    "Run output mismatch for {}.\nActual:\n{}\n\nExpected:\n{}",
                    $example, actual_trim, expected_trim
                );
            } else {
                let _ = actual;
            }
