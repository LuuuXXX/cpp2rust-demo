...
**关键约束**：
- **Linux / macOS**：依赖 `LD_PRELOAD` 机制，将 `libhook.so` 注入编译器进程；`hook.cpp` 源码已内嵌进工具 binary（`include_str!`），首次运行时自动解压到用户数据目录并编译，后续自动跳过重编译。
- **Windows**：使用 **PATH 首位注入 wrapper exe** 替代 LD_PRELOAD；工具自动将 `hook-wrapper.exe` 解压到 `%LOCALAPPDATA%\cpp2rust-demo\hook\`，并在构建命令运行期间将其作为编译器别名（`cl.exe` / `clang-cl.exe` / `g++.exe`）注入 PATH 首位；用户接口 `init -- <BUILD_CMD>` / `merge` **完全不变**，所有 PATH 修改仅作用于子进程，不影响父 shell。
...
> **Windows 前置条件**：需要以下任一 C++ 工具链（用于预处理 `.cpp` 文件）：
> - [LLVM/Clang](https://releases.llvm.org/)（推荐，提供 `clang-cl.exe`）
> - MSVC Build Tools（提供 `cl.exe`，需安装 Visual Studio 或 Build Tools for Visual Studio）
> - MinGW-w64（提供 `g++.exe`，如 [MSYS2](https://www.msys2.org/)）
>
> 工具本身通过 `cargo install` 安装，不需要额外配置；`hook-wrapper.exe` 已内嵌进 binary，
> 首次执行 `init` 时自动解压到 `%LOCALAPPDATA%\cpp2rust-demo\hook\`。

> **注意（Unix）**：`hook/hook.cpp` 已内嵌进 binary，无需额外文件。首次执行 `init` 时工具
...
| 场景 | 说明 |
|------|------|
| **仅支持 x86-64** | ARM Windows 等架构待验证；Apple Silicon（arm64 macOS）理论支持但未系统测试 |
...
| 优先级 | 方向 | 现状 | 目标 |
|--------|------|------|------|
| **P1** | **Windows 支持** ✅ | 已实现：PATH wrapper 替代 LD_PRELOAD，支持 cl.exe / clang-cl / MinGW，用户接口不变 | 持续完善 ARM Windows、L3/L4/L5 测试覆盖 |
