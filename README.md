...
**关键约束**：捕获阶段依赖 Linux/macOS 的 `LD_PRELOAD` / `DYLD_INSERT_LIBRARIES` 机制，Windows 尚不支持编译拦截。`hook.cpp` 源码已内嵌进工具 binary（`include_str!`），首次运行时自动解压到用户数据目录并编译，后续自动跳过重编译。生成的 Rust 项目本身（L2 编译 / L3 运行 / L5 符号验证）已在 Windows MSVC 和 MinGW 环境通过 CI 验证。
...
| 场景 | 说明 |
|------|------|
| **捕获阶段仅支持 Linux/macOS** | 编译拦截依赖 `LD_PRELOAD`（Linux）/ `DYLD_INSERT_LIBRARIES`（macOS），Windows 尚不支持；生成的 Rust 项目（L2/L3/L5）已在 Windows MSVC & MinGW 通过 CI |
...
| 优先级 | 方向 | 现状 | 目标 |
|--------|------|------|------|
| **P1** | **Windows 编译拦截** | 捕获阶段依赖 `LD_PRELOAD`（Linux）/ `DYLD_INSERT_LIBRARIES`（macOS），Windows 尚无等价机制；生成的 Rust 项目（L2/L3/L5）已在 Windows MSVC & MinGW 通过 CI | 评估 CMake compiler launcher（`CMAKE_CXX_COMPILER_LAUNCHER`）或 DLL 注入等替代方案，实现 Windows 编译拦截 |
